import{t as e}from"./rolldown-runtime-CmsrGqdA.js";import{t}from"./supabase-config-BObpT6R-.js";import{a as n,i as r,n as i,t as a}from"./revenue-C1-ol1Bl.js";var o={async createProfile(e,t){let{data:n}=await window.supabase.from(`profiles`).select(`id`).eq(`email`,e.email).maybeSingle();if(n&&n.id!==e.uid){console.warn(`Profile with email ${e.email} exists with different ID. Merging accounts.`);let{error:t}=await window.supabase.from(`profiles`).update({id:e.uid}).eq(`id`,n.id);if(t)throw t;return this.getProfile(e.uid)}let{data:r,error:i}=await window.supabase.from(`profiles`).upsert({id:e.uid,email:e.email,full_name:t||e.displayName||`Atleta`,role:`athlete`,membership_limit:2,xp:0,level:0});if(i)throw i;return r},async getProfile(e){try{let{data:t,error:n}=await window.supabase.from(`profiles`).select(`*, membership_plans(id, name, theme, price, monthly)`).eq(`id`,e).maybeSingle();if(n){n.code!==`PGRST116`&&console.warn(`[Supabase] getProfile error (ID: ${e}): ${n.message}`);let{data:t,error:r}=await window.supabase.from(`profiles`).select(`*`).eq(`id`,e).maybeSingle();return r?null:t}return t}catch(e){return console.error(`[Supabase] getProfile exception:`,e),null}},async updateProfile(e,t){let{data:n,error:r}=await window.supabase.from(`profiles`).update(t).eq(`id`,e);if(r)throw r;return n},async saveFCMToken(e,t){return this.updateProfile(e,{fcm_token:t})},async deleteUser(e){let{data:t,error:n}=await window.supabase.from(`profiles`).delete().eq(`id`,e);if(n)throw n;return t},async softDeleteUser(e){let{data:t,error:n}=await window.supabase.from(`profiles`).update({is_deleted:!0}).eq(`id`,e);if(n)throw n;return t},async getAttendance(e){let{data:t,error:n}=await window.supabase.from(`attendance`).select(`*`).eq(`user_id`,e).order(`attended_at`,{ascending:!1});if(n)throw n;return t},async logAttendance(e,t,n){let{data:r,error:i}=await window.supabase.from(`attendance`).insert({user_id:e,class_id:t,class_name:n});if(i)throw i;return r},async recordPayment(e,t){let{data:n,error:r}=await window.supabase.from(`payments`).insert({user_id:e,...t});if(r)throw r;return n},async getPayments(e){let{data:t,error:n}=await window.supabase.from(`payments`).select(`*`).eq(`user_id`,e).order(`created_at`,{ascending:!1});if(n)throw n;return t},async getAllPayments(){let{data:e,error:t}=await window.supabase.from(`payments`).select(`*, profiles(full_name)`).order(`created_at`,{ascending:!1});if(t)throw t;return e},async getAllProfiles(){let{data:e,error:t}=await window.supabase.from(`profiles`).select(`*, membership_plans(name, monthly)`).eq(`role`,`athlete`).eq(`is_deleted`,!1).order(`full_name`,{ascending:!0});if(t)throw t;return e},async getClasses(){let{data:e,error:t}=await window.supabase.from(`classes`).select(`*`);if(t)throw t;return e.map(e=>({...e,img:e.img&&e.img.includes(`photo-1599058917232-d750c1844bb7`)?`https://images.unsplash.com/photo-1552072092-7f9b8d63efcb?auto=format&fit=crop&q=80&w=500`:e.img}))},async upsertClass(e){let{data:t,error:n}=await window.supabase.from(`classes`).upsert(e);if(n)throw n;return t},async getPlans(){let{data:e,error:t}=await window.supabase.from(`membership_plans`).select(`*`).order(`sort_order`,{ascending:!0});if(t)throw t;return e},async upsertPlan(e){let{data:t,error:n}=await window.supabase.from(`membership_plans`).upsert(e);if(n)throw n;return t},async getMembershipPlans(){return this.getPlans()},async updatePlanOrder(e,t){let{error:n}=await window.supabase.from(`membership_plans`).update({sort_order:t}).eq(`id`,e);if(n)throw n},async getPendingPayments(){let{data:e,error:t}=await window.supabase.from(`payments`).select(`*`).eq(`status`,`pending`).order(`created_at`,{ascending:!1});if(t)throw t;return e},async updatePaymentStatus(e,t){let{data:n,error:r}=await window.supabase.from(`payments`).update({status:t}).eq(`id`,e);if(r)throw r;return n},async getPayment(e){let{data:t,error:n}=await window.supabase.from(`payments`).select(`*`).eq(`id`,e).single();if(n)throw n;return t},async deletePendingPayments(e){let{data:t,error:n}=await window.supabase.from(`payments`).delete().eq(`user_id`,e).eq(`status`,`pending`);if(n)throw n;return t},async deletePlan(e){let{error:t}=await window.supabase.from(`membership_plans`).delete().eq(`id`,e);if(t)throw t},async deleteClass(e){let{error:t}=await window.supabase.from(`classes`).delete().eq(`id`,e);if(t)throw t},async getReservations(e){let t=window.supabase.from(`reservations`).select(`*`);e&&(t=t.eq(`reservation_date`,e));let{data:n,error:r}=await t;if(r)throw r;return n},async getAllReservations(){let{data:e,error:t}=await window.supabase.from(`reservations`).select(`*`);if(t)throw t;return e},async getUserReservations(e){let{data:t,error:n}=await window.supabase.from(`reservations`).select(`*`).eq(`user_id`,e);if(n)throw n;return t},async createReservation(e,t,n,r){let{data:i,error:a}=await window.supabase.from(`reservations`).insert({user_id:e,class_id:t,class_name:n,reservation_date:r});if(a)throw a;return i},async deleteReservation(e,t,n){let{error:r}=await window.supabase.from(`reservations`).delete().eq(`user_id`,e).eq(`class_id`,t).eq(`reservation_date`,n);if(r)throw r},async getTournaments(e){let{data:t,error:n}=await window.supabase.from(`tournaments`).select(`*`).eq(`user_id`,e).order(`date`,{ascending:!0});if(n)throw n;return t.map(e=>({...e,img:e.img&&e.img.includes(`photo-1599058917232-d750c1844bb7`)?`https://images.unsplash.com/photo-1552072092-7f9b8d63efcb?auto=format&fit=crop&q=80&w=500`:e.img}))},async upsertTournament(e,t){let{data:n,error:r}=await window.supabase.from(`tournaments`).upsert({...t,user_id:e});if(r)throw r;return n},async deleteTournament(e){let{error:t}=await window.supabase.from(`tournaments`).delete().eq(`id`,e);if(t)throw t},async uploadAvatar(e,t){let n=`${e}-${Date.now()}`,{error:r}=await window.supabase.storage.from(`avatars`).upload(n,t,{upsert:!0});if(r)throw r;let{data:i}=window.supabase.storage.from(`avatars`).getPublicUrl(n);return i.publicUrl},async uploadPaymentReceipt(e,t){let n=`${e}/${Date.now()}_${t.name}`,{error:r}=await window.supabase.storage.from(`payments`).upload(n,t,{upsert:!0});if(r)throw r;let{data:i}=window.supabase.storage.from(`payments`).getPublicUrl(n);return i.publicUrl},async getNotifications(){let{data:e,error:t}=await window.supabase.from(`global_notifications`).select(`*`).order(`created_at`,{ascending:!1});if(t)throw t;return e||[]},async addNotification(e,t,n){let{data:r,error:i}=await window.supabase.from(`global_notifications`).insert([{title:e,message:t,type:n}]).select();if(i)throw i;return r},async deleteNotification(e){let{error:t}=await window.supabase.from(`global_notifications`).delete().eq(`id`,e);if(t)throw t;return!0},async getUserNotifications(e){let{data:t,error:n}=await window.supabase.from(`user_notifications`).select(`*`).eq(`user_id`,e).order(`created_at`,{ascending:!1});if(n)throw n;return t||[]},async getUnreadCount(e){let{count:t,error:n}=await window.supabase.from(`user_notifications`).select(`*`,{count:`exact`,head:!0}).eq(`user_id`,e).eq(`is_read`,!1);if(n)throw n;return t||0},async sendUserNotification(e,t,n,r=`direct`){let i=window.appState?.user?.uid||null,{data:a,error:o}=await window.supabase.from(`user_notifications`).insert([{user_id:e,sender_id:i,title:t,message:n,type:r}]).select();if(o)throw o;return a},async sendBulkUserNotifications(e,t,n,r=`mass`){let i=window.appState?.user?.uid||null,a=e.map(e=>({user_id:e,sender_id:i,title:t,message:n,type:r})),{data:o,error:s}=await window.supabase.from(`user_notifications`).insert(a).select();if(s)throw s;return o},async markNotificationRead(e){let{error:t}=await window.supabase.from(`user_notifications`).update({is_read:!0,read_at:new Date().toISOString()}).eq(`id`,e);if(t)throw t;return!0},async markAllNotificationsRead(e){let{error:t}=await window.supabase.from(`user_notifications`).update({is_read:!0,read_at:new Date().toISOString()}).eq(`user_id`,e).eq(`is_read`,!1);if(t)throw t;return!0},async deleteUserNotification(e){let{error:t}=await window.supabase.from(`user_notifications`).delete().eq(`id`,e);if(t)throw t;return!0},async sendBulkEmail(e,t,n,r){let{data:i,error:a}=await window.supabase.functions.invoke(`send-email`,{body:{to:e,subject:t,text:n,html:r||n.replace(/\n/g,`<br>`)}});if(a)throw a;return i}},s=(e,t)=>{let n=window.showToast||(e=>console.log(e)),r=document.getElementById(`btn-register`);r&&(r.onclick=async()=>{let r=document.getElementById(`reg-name`).value.trim(),i=document.getElementById(`reg-email`).value.trim(),a=document.getElementById(`reg-password`).value;if(!r||!i||!a)return n(`Por favor llena todos los campos ⚠️`,`#eab308`);try{n(`Creando cuenta... 🥋`);let{data:s,error:c}=await window.supabase.auth.signUp({email:i,password:a,options:{data:{full_name:r}}});if(c)throw c;let l={...s.user,uid:s.user.id};await o.createProfile(l,r),n(`¡Cuenta creada con éxito! 🚀`),setTimeout(()=>{let n=document.getElementById(`auth-screen`),r=document.getElementById(`membership-selection-screen`);n&&n.classList.add(`hidden`),r?(t(),r.classList.remove(`hidden`)):(document.getElementById(`app-container`).classList.remove(`hidden`),e(`dashboard`))},1500)}catch(e){console.error(e);let t=`Error al registrar la cuenta`;e.code===`auth/invalid-email`?t=`El correo no tiene un formato válido.`:e.code===`auth/email-already-in-use`?t=`Este correo electrónico ya está registrado.`:e.code===`auth/weak-password`?t=`La contraseña es muy débil (mín. 6 caracteres).`:e.message&&(t+=`: `+e.message),n(t+` ❌`,`#ef4444`)}});let i=document.getElementById(`go-to-register`),a=document.getElementById(`go-to-login`),s=document.getElementById(`login-form-container`),c=document.getElementById(`register-form-container`);i&&(i.onclick=()=>{s.classList.add(`hidden`),c.classList.remove(`hidden`)}),a&&(a.onclick=()=>{c.classList.add(`hidden`),s.classList.remove(`hidden`)});let l=document.getElementById(`btn-login`);l&&(l.onclick=async()=>{let e=document.getElementById(`login-email`),t=document.getElementById(`login-password`),r=e.value,i=t.value;if(r.toLowerCase().trim()===`admin`&&(r=`admin@amaru.app`,n(`Acceso Maestro 🛡️`)),!r||!i)return n(`Faltan datos ⚠️`,`#eab308`);try{let{error:e}=await window.supabase.auth.signInWithPassword({email:r,password:i});if(e)throw e;n(`¡Bienvenido! 🥋`)}catch(e){n(`Error de acceso: `+e.message,`#ef4444`)}});let u=document.getElementById(`btn-google-login`);u&&(u.onclick=async()=>{try{n(`Conectando con Google... 🚀`);let{data:e,error:t}=await window.supabase.auth.signInWithOAuth({provider:`google`,options:{redirectTo:window.location.origin}});if(t)throw t}catch(e){n(`Error al conectar con Google ❌`,`#ef4444`),console.error(`Google Auth Error:`,e)}});let d=document.getElementById(`btn-forgot-password`);d&&(d.onclick=async()=>{let e=document.getElementById(`login-email`).value.trim();if(!e)return n(`Ingresa tu email para restablecer contraseña ⚠️`,`#eab308`);try{n(`Enviando enlace... 📧`);let{error:t}=await window.supabase.auth.resetPasswordForEmail(e,{redirectTo:window.location.origin+`/app/`});if(t)throw t;n(`Email de restablecimiento enviado ✅`,`#22c55e`)}catch(e){n(`Error: `+e.message,`#ef4444`)}});let f=document.getElementById(`btn-logout`);f&&(f.onclick=async()=>{localStorage.removeItem(`isAdminMode`),await window.supabase.auth.signOut(),window.location.reload()})},c={plans:[],classes:[],tournaments:[],userProfile:null};window.appState=c;var l=(e,t=8e3,n=`Operación`)=>new Promise((r,i)=>{let a=setTimeout(()=>{i(Error(`${n}: Tiempo de espera agotado (${t}ms)`))},t);e.then(e=>{clearTimeout(a),r(e)}).catch(e=>{clearTimeout(a),i(e)})}),u=async(e,t={})=>{let{maxRetries:n=3,delayMs:r=1e3,context:i=`Operación`}=t,a;for(let t=0;t<=n;t++)try{return t>0&&(console.log(`[Retry] ${i} - Intento ${t+1}/${n+1}`),await new Promise(e=>setTimeout(e,r*t))),await e()}catch(e){if(a=e,console.warn(`[Retry] ${i} falló en intento ${t+1}:`,e.message),e.status&&e.status>=400&&e.status<500)throw e}throw a},d=e=>{if(!e)return!1;let t=(e.message||``).toLowerCase();return t.includes(`network`)||t.includes(`timeout`)||t.includes(`fetch`)||t.includes(`abort`)||t.includes(`failed`)||t.includes(`tiempo de espera`)},f=(function(){let e=typeof document<`u`&&document.createElement(`link`).relList;return e&&e.supports&&e.supports(`modulepreload`)?`modulepreload`:`preload`})(),p=function(e){return`/`+e},m={},h=function(e,t,n){let r=Promise.resolve();if(t&&t.length>0){let e=document.getElementsByTagName(`link`),i=document.querySelector(`meta[property=csp-nonce]`),a=i?.nonce||i?.getAttribute(`nonce`);function o(e){return Promise.all(e.map(e=>Promise.resolve(e).then(e=>({status:`fulfilled`,value:e}),e=>({status:`rejected`,reason:e}))))}r=o(t.map(t=>{if(t=p(t,n),t in m)return;m[t]=!0;let r=t.endsWith(`.css`),i=r?`[rel="stylesheet"]`:``;if(n)for(let n=e.length-1;n>=0;n--){let i=e[n];if(i.href===t&&(!r||i.rel===`stylesheet`))return}else if(document.querySelector(`link[href="${t}"]${i}`))return;let o=document.createElement(`link`);if(o.rel=r?`stylesheet`:f,r||(o.as=`script`),o.crossOrigin=``,o.href=t,a&&o.setAttribute(`nonce`,a),document.head.appendChild(o),r)return new Promise((e,n)=>{o.addEventListener(`load`,e),o.addEventListener(`error`,()=>n(Error(`Unable to preload CSS for ${t}`)))})}))}function i(e){let t=new Event(`vite:preloadError`,{cancelable:!0});if(t.payload=e,window.dispatchEvent(t),!t.defaultPrevented)throw e}return r.then(t=>{for(let e of t||[])e.status===`rejected`&&i(e.reason);return e().catch(i)})},g=[],_=[],v=[],y=`resumen`,b={search:``,status:[`pending`,`approved`,`rejected`],dateFrom:``,dateTo:``,method:``,minAmount:``,maxAmount:``},x={field:`created_at`,dir:`desc`},S=1,C=25,w=`all`,ee=()=>document.getElementById(`admin-content-area`),T=e=>e==null?`$0`:`$`+Math.round(e).toLocaleString(`es-CL`),te=e=>e?new Date(e).toLocaleDateString(`es-CL`,{day:`2-digit`,month:`short`,year:`numeric`}):`—`,E=e=>[`Enero`,`Febrero`,`Marzo`,`Abril`,`Mayo`,`Junio`,`Julio`,`Agosto`,`Septiembre`,`Octubre`,`Noviembre`,`Diciembre`][e]||``,D=()=>new Date().toISOString().split(`T`)[0],O=e=>{if(!e)return null;let t=[`enero`,`febrero`,`marzo`,`abril`,`mayo`,`junio`,`julio`,`agosto`,`septiembre`,`octubre`,`noviembre`,`diciembre`],n=e.toLowerCase(),r=t.findIndex(e=>n.includes(e)),i=e.match(/(\d{4})/);return r===-1||!i?null:{month:r,year:parseInt(i[1])}},k=async()=>{try{let{data:e,error:t}=await window.supabase.from(`payments`).select(`*, profiles(full_name, email, role, membership_status, membership_expiry, membership_plan_id)`).order(`created_at`,{ascending:!1});if(t)throw t;let{data:n,error:r}=await window.supabase.from(`profiles`).select(`*`);if(r)throw r;let{data:i,error:a}=await window.supabase.from(`membership_plans`).select(`*`);if(a)throw a;g=e||[],_=n||[],v=i||[]}catch(e){console.error(`[Payments] Error fetching data:`,e),window.showToast&&window.showToast(`Error cargando datos de pagos`,`#ef4444`)}},ne=async()=>{let e=ee(),t=document.getElementById(`admin-revenue-section`);t&&t.classList.add(`hidden`),e.innerHTML=`
        ...
        <div class="glass-premium p-20" id="payments-dashboard">
            ...
            <div style="display:flex; gap:8px; margin-bottom:20px; border-bottom:1px solid rgba(255,255,255,0.08); padding-bottom:12px; flex-wrap:wrap;">
                <button id="tab-resumen" class="btn-glass" style="padding:8px 18px; font-size:0.8rem; font-weight:700; ${y===`resumen`?`background:var(--accent-purple); color:#fff; border-color:var(--accent-purple);`:``}">
                    <i data-lucide="layout-dashboard" style="width:14px; vertical-align:middle; margin-right:4px;"></i> Resumen
                </button>
                <button id="tab-transacciones" class="btn-glass" style="padding:8px 18px; font-size:0.8rem; font-weight:700; ${y===`transacciones`?`background:var(--accent-purple); color:#fff; border-color:var(--accent-purple);`:``}">
                    <i data-lucide="list" style="width:14px; vertical-align:middle; margin-right:4px;"></i> Transacciones
                </button>
                <button id="tab-cobranza" class="btn-glass" style="padding:8px 18px; font-size:0.8rem; font-weight:700; ${y===`cobranza`?`background:var(--accent-purple); color:#fff; border-color:var(--accent-purple);`:``}">
    ...
    `,window.lucide&&window.lucide.createIcons(),document.getElementById(`tab-resumen`).onclick=()=>{y=`resumen`,A()},document.getElementById(`tab-transacciones`).onclick=()=>{y=`transacciones`,A()},document.getElementById(`tab-cobranza`).onclick=()=>{y=`cobranza`,A()};let t=document.getElementById(`link-revenue-advanced`);t&&(t.onclick=e=>{e.preventDefault(),h(()=>import(`./revenue-C1-ol1Bl.js`).then(e=>e.r).then(e=>{e.renderRevenueSection&&e.renderRevenueSection()}),[])});let n=document.getElementById(`payments-view-content`);y===`resumen`?re(n):y===`transacciones`?ae(n):se(n)},re=e=>{let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=g.filter(e=>{let t=new Date(e.created_at);return t.getFullYear()===r&&t.getMonth()===n&&e.status===`approved`}),a=i.reduce((e,t)=>e+(parseFloat(t.amount)||0),0),o=i.length,s=o>0?a/o:0,c=n===0?11:n-1,l=n===0?r-1:r,u=g.filter(e=>{let t=new Date(e.created_at);return t.getFullYear()===l&&t.getMonth()===c&&e.status===`approved`}).reduce((e,t)=>e+(parseFloat(t.amount)||0),0),d=u>0?(a-u)/u*100:a>0?100:0,f=g.filter(e=>e.status===`pending`),p=f.reduce((e,t)=>e+(parseFloat(t.amount)||0),0),m=_.filter(e=>e.membership_status===`active`),h=m.length,y=m.filter(e=>{let t=g.filter(t=>t.user_id===e.id&&t.status===`approved`).sort((e,t)=>new Date(t.created_at)-new Date(e.created_at))[0];if(!t)return!1;let i=O(t.coverage_month);return i?i.month===n&&i.year===r:!1}).length,b=h>0?(y/h*100).toFixed(0):0,x=f.filter(e=>(t-new Date(e.created_at))/(1e3*60*60*24)<=7).slice(0,5),S=m.filter(e=>{if(!e.membership_expiry)return!1;let n=Math.ceil((new Date(e.membership_expiry)-t)/(1e3*60*60*24));return n>0&&n<=5}).slice(0,5),C=[];for(let e=6;e>=0;e--){let n=new Date(t);n.setDate(n.getDate()-e);let r=n.toISOString().split(`T`)[0],i=g.filter(e=>e.created_at&&e.created_at.startsWith(r)&&e.status===`approved`);C.push({label:n.toLocaleDateString(`es-CL`,{weekday:`short`,day:`numeric`}),total:i.reduce((e,t)=>e+(parseFloat(t.amount)||0),0),count:i.length})}e.innerHTML=`
        <!-- KPIs -->
        <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(160px, 1fr)); gap:12px; margin-bottom:25px;">
            <div class="stat-mini-premium" style="background:rgba(139,92,246,0.08); border:1px solid rgba(139,92,246,0.2); padding:18px; border-radius:14px; text-align:center;">
                <span style="font-size:0.65rem; opacity:0.6; text-transform:uppercase; font-weight:700; letter-spacing:1px; display:block; margin-bottom:6px;">Ingresos ${E(n)}</span>
                <strong style="font-size:1.5rem; color:var(--accent-purple); font-weight:900;">${T(a)}</strong>
            ...
            <div class="stat-mini-premium ${f.length>0?`pulse-warning`:``}" style="background:rgba(251,191,36,0.08); border:1px solid rgba(251,191,36,0.2); padding:18px; border-radius:14px; text-align:center;">
                ...
                <span style="font-size:0.75rem; color:var(--text-gray); display:block; margin-top:4px;">${T(p)}</span>
            </div>
            <div class="stat-mini-premium" style="background:rgba(34,197,94,0.08); border:1px solid rgba(34,197,94,0.2); padding:18px; border-radius:14px; text-align:center;">
                ...
                <strong style="font-size:1.5rem; color:#22c55e; font-weight:900;">${y}/${h}</strong>
            ...
            <div class="stat-mini-premium" style="background:rgba(59,130,246,0.08); border:1px solid rgba(59,130,246,0.2); padding:18px; border-radius:14px; text-align:center;">
                ...
                <strong style="font-size:1.5rem; color:#3b82f6; font-weight:900;">${T(s)}</strong>
        ...
        <div style="margin-bottom:20px;">
            ...
            ${x.length===0&&S.length===0?`<p style="opacity:0.5; font-size:0.85rem; padding:15px;">🎉 No hay acciones pendientes. Todo está al día.</p>`:`<div style="display:flex; flex-direction:column; gap:8px;">
                    ${x.map(e=>{let n=e.profiles?.full_name||e.user_name||`Usuario`,r=T(parseFloat(e.amount)||0),i=Math.floor((t-new Date(e.created_at))/(1e3*60*60*24));return`
                        ...
                        `}).join(``)}
                    ${S.map(e=>{let n=Math.ceil((new Date(e.membership_expiry)-t)/(1e3*60*60*24)),r=v.find(t=>t.id===e.membership_plan_id);return`
    ...
    `,window.lucide&&window.lucide.createIcons(),ie(C),e.querySelectorAll(`.btn-quick-approve`).forEach(e=>{e.onclick=()=>Ae(e.getAttribute(`data-id`),`approved`)}),e.querySelectorAll(`.btn-quick-register`).forEach(e=>{e.onclick=()=>j(e.getAttribute(`data-uid`))})},ie=e=>{let t=document.getElementById(`resumenChart`);t&&(window._resumenChart&&window._resumenChart.destroy(),window._resumenChart=new Chart(t,{type:`bar`,data:{labels:e.map(e=>e.label),datasets:[{label:`Ingresos ($)`,data:e.map(e=>e.total),backgroundColor:`rgba(139, 92, 246, 0.7)`,borderColor:`rgba(139, 92, 246, 1)`,borderWidth:1,borderRadius:6}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1},tooltip:{backgroundColor:`rgba(13,13,18,0.95)`,callbacks:{label:e=>` Ingresos: ${T(e.raw)}`}}},scales:{x:{ticks:{color:`rgba(255,255,255,0.5)`,font:{size:10}},grid:{display:!1}},y:{ticks:{color:`rgba(255,255,255,0.4)`,font:{size:10},callback:e=>`$`+(e>=1e3?(e/1e3).toFixed(0)+`k`:e)},grid:{color:`rgba(255,255,255,0.06)`}}}}}))},ae=e=>{let t=g.filter(e=>{if(!b.status.includes(e.status)||b.method&&e.payment_method!==b.method||b.dateFrom&&e.created_at<b.dateFrom+`T00:00:00`||b.dateTo&&e.created_at>b.dateTo+`T23:59:59`||b.minAmount&&(parseFloat(e.amount)||0)<parseFloat(b.minAmount)||b.maxAmount&&(parseFloat(e.amount)||0)>parseFloat(b.maxAmount))return!1;if(b.search){let t=b.search.toLowerCase(),n=(e.profiles?.full_name||e.user_name||``).toLowerCase(),r=(e.profiles?.email||``).toLowerCase(),i=(e.concept||e.plan_name||``).toLowerCase();return n.includes(t)||r.includes(t)||i.includes(t)}return!0});t.sort((e,t)=>{let n,r;switch(x.field){case`amount`:n=parseFloat(e.amount)||0,r=parseFloat(t.amount)||0;break;case`name`:n=e.profiles?.full_name||``,r=t.profiles?.full_name||``;break;case`status`:n=e.status,r=t.status;break;default:n=new Date(e.created_at),r=new Date(t.created_at)}return x.dir===`asc`?n>r?1:-1:n<r?1:-1});let n=t.reduce((e,t)=>e+(parseFloat(t.amount)||0),0),r=Math.max(1,Math.ceil(t.length/C));S=Math.min(S,r);let i=(S-1)*C,a=t.slice(i,i+C);e.innerHTML=`
        <!-- Filtros -->
        <div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:16px; padding:12px; background:rgba(255,255,255,0.03); border-radius:10px; border:1px solid rgba(255,255,255,0.06); align-items:center;">
            <input type="text" id="tx-search" placeholder="🔍 Buscar alumno, email o concepto..." value="${b.search}" style="flex:1; min-width:180px; background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:8px 12px; border-radius:8px; font-size:0.8rem; outline:none;">
            ...
            <div style="display:flex; gap:6px; align-items:center;">
                ${[{val:`pending`,label:`Pendiente`,color:`#fbbf24`},{val:`approved`,label:`Aprobado`,color:`#22c55e`},{val:`rejected`,label:`Rechazado`,color:`#ef4444`}].map(e=>`
                    <label style="display:flex; align-items:center; gap:4px; font-size:0.7rem; cursor:pointer; background:rgba(255,255,255,0.05); padding:4px 10px; border-radius:6px; border:1px solid ${b.status.includes(e.val)?e.color:`rgba(255,255,255,0.1)`};">
                        <input type="checkbox" class="tx-status-check" value="${e.val}" ${b.status.includes(e.val)?`checked`:``} style="accent-color:${e.color};">
                        <span style="color:${b.status.includes(e.val)?e.color:`var(--text-gray)`};">${e.label}</span>
                    </label>
                `).join(``)}
            </div>
            ...
            <select id="tx-method" style="background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:8px 10px; border-radius:8px; font-size:0.75rem; outline:none;">
                ...
                <option value="transferencia" ${b.method===`transferencia`?`selected`:``}>Transferencia Bancaria</option>
                <option value="pasarela" ${b.method===`pasarela`?`selected`:``}>Pasarela de Pago</option>
                <option value="efectivo" ${b.method===`efectivo`?`selected`:``}>Efectivo</option>
                <option value="mercadopago" ${b.method===`mercadopago`?`selected`:``}>Mercado Pago</option>
                <option value="webpay" ${b.method===`webpay`?`selected`:``}>Webpay</option>
                <option value="manual" ${b.method===`manual`?`selected`:``}>Transferencia/Manual (legado)</option>
            </select>

            <div style="display:flex; gap:6px; align-items:center;">
                <input type="number" id="tx-min" placeholder="Min $" value="${b.minAmount}" style="width:80px; background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:8px 10px; border-radius:8px; font-size:0.75rem; outline:none;">
                <span style="opacity:0.5; font-size:0.75rem;">-</span>
                <input type="number" id="tx-max" placeholder="Max $" value="${b.maxAmount}" style="width:80px; background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:8px 10px; border-radius:8px; font-size:0.75rem; outline:none;">
            </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px; flex-wrap:wrap; gap:10px;">
            <span style="font-size:0.8rem; color:var(--text-gray);">Mostrando <strong style="color:white;">${t.length}</strong> transacciones | Total filtrado: <strong style="color:var(--accent-purple);">${T(n)}</strong></span>
                ...
                <tbody>
                    ${a.length===0?`<tr><td colspan="8" style="padding:30px; text-align:center; opacity:0.5;">No hay transacciones que coincidan con los filtros.</td></tr>`:a.map(e=>{let t=e.profiles?.full_name||e.user_name||`N/A`,n=e.profiles?.email||``,r=e.concept||e.plan_name||`Membresía`,i=parseFloat(e.amount)||0,a=e.payment_method||`manual`,o={transferencia:`Transferencia Bancaria`,pasarela:`Pasarela de Pago`,efectivo:`Efectivo`,mercadopago:`Mercado Pago`,webpay:`Webpay`,manual:`Transferencia/Manual`}[a]||a,s=e.status===`approved`?`#22c55e`:e.status===`pending`?`#fbbf24`:`#ef4444`,c=e.status===`approved`?`Aprobado`:e.status===`pending`?`Pendiente`:`Rechazado`,l=e.status===`approved`?`rgba(34,197,94,0.15)`:e.status===`pending`?`rgba(251,191,36,0.15)`:`rgba(239,68,68,0.15)`;return`
                            <tr style="border-bottom:1px solid rgba(255,255,255,0.04); transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
                                <td style="padding:10px 12px; white-space:nowrap;">${te(e.created_at)}</td>
                                <td style="padding:10px 12px;">
                                    <strong>${t}</strong>
                                    <span style="font-size:0.7rem; color:var(--text-gray); display:block;">${n}</span>
                                </td>
                                <td style="padding:10px 12px;">${r}</td>
                                <td style="padding:10px 12px; text-align:right; font-weight:700;">${T(i)}</td>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
            <div style="display:flex; gap:8px; align-items:center;">
                ...
                <select id="tx-per-page" style="background:rgba(255,255,255,0.05); border:1px solid var(--glass-border); color:white; padding:5px 8px; border-radius:6px; font-size:0.75rem; outline:none;">
                    <option value="25" ${C===25?`selected`:``}>25</option>
                    <option value="50" ${C===50?`selected`:``}>50</option>
                    <option value="100" ${C===100?`selected`:``}>100</option>
                </select>
            </div>
            <div style="display:flex; gap:6px; align-items:center;">
                <button id="tx-prev-page" class="btn-glass-small" ${S<=1?`disabled style="opacity:0.3;"`:``} style="font-size:0.75rem;"><i data-lucide="chevron-left" style="width:14px;"></i></button>
                <span style="font-size:0.8rem;">Página <strong>${S}</strong> de ${r}</span>
                <button id="tx-next-page" class="btn-glass-small" ${S>=r?`disabled style="opacity:0.3;"`:``} style="font-size:0.75rem;"><i data-lucide="chevron-right" style="width:14px;"></i></button>
            </div>
        </div>
    `,window.lucide&&window.lucide.createIcons(),oe(e,t)},oe=(e,t)=>{let n=e.querySelector(`#tx-search`);if(n){let e;n.oninput=()=>{clearTimeout(e),e=setTimeout(()=>{b.search=n.value,S=1,A()},300)}}let i=e.querySelector(`#tx-date-preset`);i&&(i.onchange=()=>{let e=new Date,t=i.value;if(b.dateFrom=``,b.dateTo=``,t===`today`)b.dateFrom=D(),b.dateTo=D();else if(t===`week`){let t=new Date(e);t.setDate(t.getDate()-7),b.dateFrom=t.toISOString().split(`T`)[0],b.dateTo=D()}else if(t===`month`)b.dateFrom=`${e.getFullYear()}-${String(e.getMonth()+1).padStart(2,`0`)}-01`,b.dateTo=D();else if(t===`lastmonth`){let t=new Date(e.getFullYear(),e.getMonth()-1,1);b.dateFrom=t.toISOString().split(`T`)[0];let n=new Date(e.getFullYear(),e.getMonth(),0);b.dateTo=n.toISOString().split(`T`)[0]}S=1,A()}),e.querySelectorAll(`.tx-status-check`).forEach(t=>{t.onchange=()=>{let t=Array.from(e.querySelectorAll(`.tx-status-check:checked`)).map(e=>e.value);b.status=t.length?t:[`pending`,`approved`,`rejected`],S=1,A()}});let a=e.querySelector(`#tx-method`);a&&(a.onchange=()=>{b.method=a.value,S=1,A()});let o=e.querySelector(`#tx-min`),s=e.querySelector(`#tx-max`);o&&(o.onchange=()=>{b.minAmount=o.value,S=1,A()}),s&&(s.onchange=()=>{b.maxAmount=s.value,S=1,A()});let c=e.querySelector(`#tx-clear-filters`);c&&(c.onclick=()=>{b={search:``,status:[`pending`,`approved`,`rejected`],dateFrom:``,dateTo:``,method:``,minAmount:``,maxAmount:``},S=1,A()}),e.querySelectorAll(`th[data-sort]`).forEach(e=>{e.onclick=()=>{let t=e.getAttribute(`data-sort`);x.field===t?x.dir=x.dir===`asc`?`desc`:`asc`:(x.field=t,x.dir=`desc`),A()}});let l=e.querySelector(`#tx-prev-page`),u=e.querySelector(`#tx-next-page`),d=e.querySelector(`#tx-per-page`);l&&(l.onclick=()=>{S>1&&(S--,A())}),u&&(u.onclick=()=>{S++,A()}),d&&(d.onchange=()=>{C=parseInt(d.value),S=1,A()}),e.querySelectorAll(`.tx-approve-btn`).forEach(e=>{e.onclick=()=>Ae(e.getAttribute(`data-id`),`approved`)}),e.querySelectorAll(`.tx-edit-btn`).forEach(e=>{e.onclick=async()=>{let t=prompt(`Nuevo monto (deja vacío para cancelar):`);if(t!==null&&t.trim()!==``)try{await window.supabase.from(`payments`).update({amount:parseFloat(t)}).eq(`id`,e.getAttribute(`data-id`)),window.showToast(`Monto actualizado ✅`,`#22c55e`),await k(),A()}catch{window.showToast(`Error al editar`,`#ef4444`)}}}),e.querySelectorAll(`.tx-revert-btn`).forEach(e=>{e.onclick=()=>Ae(e.getAttribute(`data-id`),`pending`)}),e.querySelectorAll(`.tx-delete-btn`).forEach(e=>{e.onclick=async()=>{if(confirm(`¿Eliminar este registro de pago?`))try{await window.supabase.from(`payments`).delete().eq(`id`,e.getAttribute(`data-id`)),window.showToast(`Registro eliminado ✅`,`#22c55e`),await k(),A()}catch{window.showToast(`Error al eliminar`,`#ef4444`)}}});let f=e.querySelector(`#tx-export-btn`);f&&(f.onclick=()=>{let n=e.querySelector(`#tx-export-format`)?.value||`csv`;r(n,t.map(e=>[te(e.created_at),e.profiles?.full_name||e.user_name||`N/A`,e.profiles?.email||``,e.concept||e.plan_name||`Membresía`,parseFloat(e.amount||0).toFixed(0),e.payment_method||`manual`,e.status===`approved`?`Aprobado`:e.status===`pending`?`Pendiente`:`Rechazado`,e.coverage_month||``]),[`Fecha`,`Alumno`,`Email`,`Concepto`,`Monto`,`Método`,`Estado`,`Cobertura`],`Transacciones_Amaru_${D()}`),window.showToast&&window.showToast(`Exportado (${n.toUpperCase()}) ✅`,`#22c55e`)})},se=e=>{let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=_.filter(e=>e.membership_status===`active`||e.membership_status===`frozen`).map(e=>{let i=g.filter(t=>t.user_id===e.id&&t.status===`approved`).sort((e,t)=>new Date(t.created_at)-new Date(e.created_at))[0]||null,a=i?O(i.coverage_month):null,o=`none`,s=`Sin registro`,c=`#9ca3af`,l=`rgba(156,163,175,0.15)`;if(i)if(a&&a.month===n&&a.year===r)o=`ok`,s=`Al día`,c=`#22c55e`,l=`rgba(34,197,94,0.15)`;else{let n=e.membership_expiry?new Date(e.membership_expiry):null,r=n?Math.ceil((n-t)/(1e3*60*60*24)):null;r!==null&&r>0&&r<=5?(o=`warning`,s=`Por vencer`,c=`#fbbf24`,l=`rgba(251,191,36,0.15)`):r!==null&&r<=0?(o=`overdue`,s=`Moroso`,c=`#ef4444`,l=`rgba(239,68,68,0.15)`):(o=`warning`,s=`Pendiente`,c=`#fbbf24`,l=`rgba(251,191,36,0.15)`)}let u=v.find(t=>t.id===e.membership_plan_id);return{profile:e,lastPayment:i,coverageMonth:i?.coverage_month||`—`,status:o,statusLabel:s,statusColor:c,statusBg:l,planName:u?.name||`Sin plan`,planPrice:u?.price||0}}),a=i;w!==`all`&&(a=i.filter(e=>e.status===w));let o={ok:0,warning:0,overdue:0,none:0};i.forEach(e=>o[e.status]++);let s=i.length,c=s>0?(o.ok/s*100).toFixed(0):0;e.innerHTML=`
        <!-- KPIs Cobranza -->
        <div style="display:flex; flex-wrap:wrap; gap:10px; margin-bottom:20px; padding:15px; background:rgba(255,255,255,0.03); border-radius:12px; border:1px solid rgba(255,255,255,0.06); align-items:center;">
            ...
            <div style="display:flex; gap:6px; flex-wrap:wrap;">
                <button class="cov-filter-btn btn-glass-small ${w===`all`?`active`:``}" data-filter="all" style="font-size:0.75rem; ${w===`all`?`background:rgba(255,255,255,0.1); color:white; border-color:white;`:``}">
                    Todos (${s})
                </button>
                <button class="cov-filter-btn btn-glass-small ${w===`ok`?`active`:``}" data-filter="ok" style="font-size:0.75rem; ${w===`ok`?`background:rgba(34,197,94,0.2); color:#22c55e; border-color:#22c55e;`:`color:#22c55e; border-color:rgba(34,197,94,0.3);`}">
                    🟢 Al día (${o.ok})
                </button>
                <button class="cov-filter-btn btn-glass-small ${w===`warning`?`active`:``}" data-filter="warning" style="font-size:0.75rem; ${w===`warning`?`background:rgba(251,191,36,0.2); color:#fbbf24; border-color:#fbbf24;`:`color:#fbbf24; border-color:rgba(251,191,36,0.3);`}">
                    🟡 Por vencer (${o.warning})
                </button>
                <button class="cov-filter-btn btn-glass-small ${w===`overdue`?`active`:``}" data-filter="overdue" style="font-size:0.75rem; ${w===`overdue`?`background:rgba(239,68,68,0.2); color:#ef4444; border-color:#ef4444;`:`color:#ef4444; border-color:rgba(239,68,68,0.3);`}">
                    🔴 Morosos (${o.overdue})
                </button>
                <button class="cov-filter-btn btn-glass-small ${w===`none`?`active`:``}" data-filter="none" style="font-size:0.75rem; ${w===`none`?`background:rgba(156,163,175,0.2); color:#9ca3af; border-color:#9ca3af;`:`color:#9ca3af; border-color:rgba(156,163,175,0.3);`}">
                    ⚪ Sin registro (${o.none})
                </button>
            </div>
            <div style="min-width:150px; text-align:right;">
                <div style="font-size:0.75rem; color:var(--text-gray); margin-bottom:4px;">Cobertura ${E(n)} ${r}</div>
            ...
            ${a.length===0?`<p style="opacity:0.5; font-size:0.85rem; padding:20px; text-align:center;">No hay alumnos en esta categoría.</p>`:a.map(e=>{let n=e.profile,r=n.membership_expiry?Math.ceil((new Date(n.membership_expiry)-t)/(1e3*60*60*24)):null;return`
                    <div class="admin-item-card glass" style="flex-direction:row; align-items:center; gap:12px; padding:12px 15px; border:1px solid rgba(255,255,255,0.05); border-left:3px solid ${e.statusColor};">
                        ...
                        <div style="text-align:right; flex-shrink:0; min-width:120px;">
                            ...
                            <strong style="font-size:0.85rem; color:white;">${e.lastPayment?T(parseFloat(e.lastPayment.amount)||0):`—`}</strong>
    ...
    `,window.lucide&&window.lucide.createIcons(),e.querySelectorAll(`.cov-filter-btn`).forEach(e=>{e.onclick=()=>{w=e.getAttribute(`data-filter`),A()}}),e.querySelectorAll(`.btn-register-payment`).forEach(e=>{e.onclick=()=>j(e.getAttribute(`data-uid`))})},j=e=>{let t=_.find(t=>t.id===e),n=v.find(e=>e.id===t?.membership_plan_id),r=n?.price||``,i=n?.name||`Membresía`,o=new Date,s=E(o.getMonth()),c=o.getFullYear(),l=document.createElement(`div`);l.className=`glass-container`,l.style.cssText=`
    ...
    `,document.body.appendChild(l),window.lucide&&window.lucide.createIcons();let u=()=>{l.style.opacity=`0`,setTimeout(()=>l.remove(),300)};l.querySelectorAll(`.close-quick-modal`).forEach(e=>{e.onclick=u}),document.getElementById(`qp-save-btn`).onclick=async()=>{let t=parseFloat(document.getElementById(`qp-amount`).value),n=document.getElementById(`qp-concept`).value.trim(),r=document.getElementById(`qp-method`).value,i=document.getElementById(`qp-month`).value,o=document.getElementById(`qp-year`).value;if(!t||t<=0){window.showToast(`Ingresa un monto válido`,`#ef4444`);return}try{window.showToast(`Registrando pago...`);let{error:s}=await window.supabase.from(`payments`).insert({user_id:e,amount:t,concept:n,status:`approved`,payment_method:r,coverage_month:`${i} ${o}`,created_at:new Date().toISOString()});if(s)throw s;let c=new Date;c.setDate(c.getDate()+30),await window.supabase.from(`profiles`).update({membership_status:`active`,membership_expiry:c.toISOString(),is_frozen:!1}).eq(`id`,e),window.showToast(`Pago registrado ✅`,`#22c55e`),u(),await k(),A(),a()}catch(e){console.error(e),window.showToast(`Error al registrar pago`,`#ef4444`)}}},M=e({renderAdminMembers:()=>G}),N=`../images/unknow.png`,P=[],F=[],ce=[],I=`dashboard`,L={search:``,status:[],plan:``,minDaysLeft:``,maxDaysLeft:``,minLastAttendance:``},R={field:`full_name`,dir:`asc`},z=1,B=25,V=`all`,H=`all`,U=()=>document.getElementById(`admin-content-area`),W=e=>e?new Date(e).toLocaleDateString(`es-CL`,{day:`2-digit`,month:`short`,year:`numeric`}):`—`,le=e=>[`Enero`,`Febrero`,`Marzo`,`Abril`,`Mayo`,`Junio`,`Julio`,`Agosto`,`Septiembre`,`Octubre`,`Noviembre`,`Diciembre`][e]||``,ue=async()=>{try{let e=await window.supabase.from(`profiles`).select(`*`).eq(`is_deleted`,!1),t=await window.supabase.from(`reservations`).select(`*`),n=await window.supabase.from(`membership_plans`).select(`*`),r=await window.supabase.from(`payments`).select(`*`);P=e.data||[],F=t.data||[],n.data,ce=r.data||[]}catch(e){console.error(`[Members] Error fetching data:`,e),window.showToast&&window.showToast(`Error cargando datos de socios`,`#ef4444`)}},G=async()=>{let e=U(),t=document.getElementById(`admin-revenue-section`);t&&t.classList.add(`hidden`),e.innerHTML=`
        <div class="glass-premium p-20">
            <div class="p-20 text-center">
                <i data-lucide="loader" class="spin"></i>
                <p style="margin-top:10px; opacity:0.7; font-size:0.85rem;">Cargando control de socios...</p>
            </div>
        </div>`,window.lucide&&window.lucide.createIcons(),await ue(),K()},K=()=>{let e=U();if(!e)return;e.innerHTML=`
    ...
    `,window.lucide&&window.lucide.createIcons(),document.getElementById(`tab-dashboard`).onclick=()=>{I=`dashboard`,K()},document.getElementById(`tab-directorio`).onclick=()=>{I=`directorio`,K()},document.getElementById(`tab-retencion`).onclick=()=>{I=`retencion`,K()},document.getElementById(`tab-comunicaciones`).onclick=()=>{I=`comunicaciones`,K()},document.getElementById(`btn-add-member-admin`).onclick=()=>Z();let t=document.getElementById(`members-view-content`);I===`dashboard`?de(t):I===`directorio`?me(t):I===`retencion`?ge(t):_e(t)},de=e=>{let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=P.map(e=>ve(e,t,n,r)),a=i.length,o=i.filter(e=>e._status===`Activo`).length,s=i.filter(e=>e._status===`Inactivo`).length,c=i.filter(e=>e._status===`Moroso`).length,l=i.filter(e=>e._status===`Congelado`).length,u=i.filter(e=>{let t=e.created_at?new Date(e.created_at):null;return t&&t.getMonth()===n&&t.getFullYear()===r}).length,d=i.length>0?Math.round(i.reduce((e,n)=>{let r=n.created_at?new Date(n.created_at):t;return e+Math.floor((t-r)/(1e3*60*60*24))},0)/i.length):0,f=a>0?(o/a*100).toFixed(0):0,p=[...i].filter(e=>e.created_at).sort((e,t)=>new Date(t.created_at)-new Date(e.created_at)).slice(0,5),m=[...ce].filter(e=>e.status===`approved`).sort((e,t)=>new Date(t.created_at)-new Date(e.created_at)).slice(0,5),h=[];for(let e=5;e>=0;e--){let t=new Date(r,n-e,1),a=t.getMonth(),o=t.getFullYear(),s=i.filter(e=>{let t=e.created_at?new Date(e.created_at):null;return t&&t.getMonth()===a&&t.getFullYear()===o}).length;h.push({label:le(a).substring(0,3),count:s})}let g={};i.forEach(e=>{let t=e.membership_plans?.name||`Sin Plan`;g[t]=(g[t]||0)+1});let _=[];for(let e=6;e>=0;e--){let n=new Date(t);n.setDate(n.getDate()-e);let r=n.toISOString().split(`T`)[0],i=F.filter(e=>e.reservation_date===r).length;_.push({label:n.toLocaleDateString(`es-CL`,{weekday:`short`,day:`numeric`}),count:i})}let v=i.filter(e=>e._status===`Activo`&&e._daysLeft!==null&&e._daysLeft>0&&e._daysLeft<=5).slice(0,5),y=i.filter(e=>e._status===`Activo`&&e._daysSinceLastAttendance>14&&e._daysSinceLastAttendance!==-1).slice(0,5),b=i.filter(e=>e._status===`Moroso`).slice(0,5);e.innerHTML=`
        <style>
                ...
                <div style="display:flex; flex-direction:column; gap:8px;">
                    ${p.length===0?`<p style="opacity:0.5; font-size:0.8rem; padding:10px;">Sin registros recientes</p>`:p.map(e=>`
                            <div style="display:flex; align-items:center; gap:10px; padding:8px; background:rgba(255,255,255,0.03); border-radius:8px; border:1px solid rgba(255,255,255,0.05);">
                                <img src="${e.photo_url||(N===void 0?`/app/images/icon-192.png`:N)}" style="width:32px; height:32px; border-radius:50%; object-fit:cover;">
                                <div style="flex:1; min-width:0;">
        ...
        <div class="dash-section">
            ...
            ${v.length===0&&y.length===0&&b.length===0?`<p style="opacity:0.5; font-size:0.85rem; padding:15px;">🎉 No hay alertas críticas. Todo está bajo control.</p>`:`<div style="display:flex; flex-direction:column; gap:8px;">
                    ${v.map(e=>fe(e,`expiring`)).join(``)}
                    ${y.map(e=>fe(e,`noAttendance`)).join(``)}
                    ${b.map(e=>fe(e,`overdue`)).join(``)}
                </div>`}
        </div>
    `,window.lucide&&window.lucide.createIcons(),pe(h,g,_),e.querySelectorAll(`.alert-action-renew`).forEach(e=>{e.onclick=()=>ye(e.getAttribute(`data-id`))}),e.querySelectorAll(`.alert-action-register`).forEach(e=>{e.onclick=()=>j(e.getAttribute(`data-uid`))}),e.querySelectorAll(`.alert-action-edit`).forEach(e=>{e.onclick=()=>Z(e.getAttribute(`data-id`),P)})},fe=(e,t)=>{let n={expiring:{icon:`clock`,color:`#fbbf24`,bg:`rgba(251,191,36,0.08)`,border:`rgba(251,191,36,0.2)`,text:`Vence en ${e._daysLeft} días`},noAttendance:{icon:`user-x`,color:`#f97316`,bg:`rgba(249,115,22,0.08)`,border:`rgba(249,115,22,0.2)`,text:`Sin asistir hace ${e._daysSinceLastAttendance} días`},overdue:{icon:`alert-circle`,color:`#ef4444`,bg:`rgba(239,68,68,0.08)`,border:`rgba(239,68,68,0.2)`,text:`Membresía vencida`}}[t];return`
                                ...
                                <tr style="border-bottom:1px solid rgba(255,255,255,0.04); transition:background 0.2s;" onmouseover="this.style.background='rgba(255,255,255,0.02)'" onmouseout="this.style.background='transparent'">
                                    <td style="padding:10px 12px;">
                                        <div style="display:flex; align-items:center; gap:10px;">
                                            <img src="${e.photo_url||(N===void 0?`/app/images/icon-192.png`:N)}" style="width:32px; height:32px; border-radius:50%; object-fit:cover; border:1px solid rgba(255,255,255,0.1);">
                                            <div>
                                    ...
                                    <td style="padding:10px 12px; white-space:nowrap;">
                                        ${e._expiryDate?W(e._expiryDate.toISOString()):`—`}
                                    ...
                                    <td style="padding:10px 12px; white-space:nowrap;">
                                        ${e._lastAttendance?W(e._lastAttendance.toISOString()):`Nunca`}
    ...
    `,window.lucide&&window.lucide.createIcons(),he(e,i)},he=(e,t)=>{let r=e.querySelector(`#dir-search`);if(r){let e;r.oninput=()=>{clearTimeout(e),e=setTimeout(()=>{L.search=r.value,z=1,K()},300)}}e.querySelectorAll(`.dir-status-check`).forEach(t=>{t.onchange=()=>{let t=Array.from(e.querySelectorAll(`.dir-status-check:checked`)).map(e=>e.value);L.status=t,z=1,K()}});let i=e.querySelector(`#dir-plan`);i&&(i.onchange=()=>{L.plan=i.value,z=1,K()});let a=e.querySelector(`#dir-min-days`),o=e.querySelector(`#dir-max-days`);a&&(a.onchange=()=>{L.minDaysLeft=a.value,z=1,K()}),o&&(o.onchange=()=>{L.maxDaysLeft=o.value,z=1,K()});let s=e.querySelector(`#dir-min-attendance`);s&&(s.onchange=()=>{L.minLastAttendance=s.value,z=1,K()});let c=e.querySelector(`#dir-clear-filters`);c&&(c.onclick=()=>{L={search:``,status:[],plan:``,minDaysLeft:``,maxDaysLeft:``,minLastAttendance:``},z=1,K()}),e.querySelectorAll(`th[data-sort]`).forEach(e=>{e.onclick=()=>{let t=e.getAttribute(`data-sort`);R.field===t?R.dir=R.dir===`asc`?`desc`:`asc`:(R.field=t,R.dir=`asc`),K()}});let l=e.querySelector(`#dir-prev-page`),u=e.querySelector(`#dir-next-page`),d=e.querySelector(`#dir-per-page`);l&&(l.onclick=()=>{z>1&&(z--,K())}),u&&(u.onclick=()=>{z++,K()}),d&&(d.onchange=()=>{B=parseInt(d.value),z=1,K()}),e.querySelectorAll(`.dir-edit-btn`).forEach(e=>{e.onclick=()=>Z(e.getAttribute(`data-id`),P)}),e.querySelectorAll(`.dir-renew-btn`).forEach(e=>{e.onclick=()=>ye(e.getAttribute(`data-id`))}),e.querySelectorAll(`.dir-delete-btn`).forEach(e=>{e.onclick=()=>Ve(e.getAttribute(`data-id`))});let f=e.querySelector(`#dir-export-btn`);f&&(f.onclick=()=>{let r=e.querySelector(`#dir-export-format`)?.value||`csv`;n(t,r),window.showToast&&window.showToast(`Exportado (${r.toUpperCase()}) ✅`,`#22c55e`)})},ge=e=>{let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=P.map(e=>ve(e,t,n,r)).map(e=>{let n=0;return e._status===`Inactivo`&&(n+=80),e._status===`Moroso`&&(n+=60),e._daysSinceLastAttendance>30?n+=40:e._daysSinceLastAttendance>14?n+=25:e._daysSinceLastAttendance>7&&(n+=10),e._daysLeft!==null&&e._daysLeft<=5&&e._daysLeft>0&&(n+=30),e._monthRes<=1&&t.getDate()>15&&(n+=15),e._status===`Congelado`&&(n+=20),{...e,_churnScore:Math.min(100,n)}}),a=i;V===`high`?a=i.filter(e=>e._churnScore>=60):V===`medium`?a=i.filter(e=>e._churnScore>=30&&e._churnScore<60):V===`low`?a=i.filter(e=>e._churnScore>0&&e._churnScore<30):V===`inactive`&&(a=i.filter(e=>e._status===`Inactivo`)),a.sort((e,t)=>t._churnScore-e._churnScore);let o={high:0,medium:0,low:0,inactive:0};i.forEach(e=>{e._status===`Inactivo`?o.inactive++:e._churnScore>=60?o.high++:e._churnScore>=30?o.medium++:e._churnScore>0&&o.low++}),e.innerHTML=`
    ...
    `,window.lucide&&window.lucide.createIcons(),e.querySelectorAll(`.risk-filter-btn`).forEach(e=>{e.onclick=()=>{V=e.getAttribute(`data-filter`),K()}}),e.querySelectorAll(`.risk-action-edit`).forEach(e=>{e.onclick=()=>Z(e.getAttribute(`data-id`),P)}),e.querySelectorAll(`.risk-action-register`).forEach(e=>{e.onclick=()=>j(e.getAttribute(`data-uid`))})},_e=e=>{let t=new Date,n=t.getMonth(),r=t.getFullYear(),i=P.map(e=>ve(e,t,n,r)),a=i;H!==`all`&&(a=i.filter(e=>H===`active`?e._status===`Activo`:H===`inactive`?e._status===`Inactivo`:H===`moroso`?e._status===`Moroso`:H===`expiring`?e._status===`Activo`&&e._daysLeft!==null&&e._daysLeft>0&&e._daysLeft<=7:H===`noAttendance`?e._status===`Activo`&&e._daysSinceLastAttendance>14:!0)),[...new Set(P.map(e=>e.membership_plans?.name).filter(Boolean))],e.innerHTML=`
                ...
                <div style="max-height:500px; overflow-y:auto; border:1px solid rgba(255,255,255,0.06); border-radius:12px; padding:10px;">
                    ${a.length===0?`<p style="opacity:0.5; font-size:0.85rem; text-align:center; padding:20px;">Selecciona un segmento para ver destinatarios.</p>`:a.slice(0,50).map(e=>`
                            <div style="display:flex; align-items:center; gap:10px; padding:8px; border-bottom:1px solid rgba(255,255,255,0.03);">
                                <img src="${e.photo_url||(N===void 0?`/app/images/icon-192.png`:N)}" style="width:28px; height:28px; border-radius:50%; object-fit:cover;">
                                <div style="flex:1; min-width:0;">
    ...
    `,window.lucide&&window.lucide.createIcons(),e.querySelectorAll(`.comm-segment-btn`).forEach(e=>{e.onclick=()=>{H=e.getAttribute(`data-segment`),K()}});let o=e.querySelector(`#comm-template`),s=e.querySelector(`#comm-message`),c={payment_reminder:`Hola {nombre},
...
Equipo Amaru 🥋`};o&&s&&(o.onchange=()=>{let e=c[o.value];e&&(s.value=e)});let l=e.querySelector(`#btn-send-comm`);l&&(l.onclick=async()=>{if(!window.SupabaseService){console.error(`[Comunications] SupabaseService not available`),window.showToast(`El servicio de comunicaciones no está disponible. Recarga la página.`,`#ef4444`);return}let t=s?.value?.trim();if(!t){window.showToast(`Escribe un mensaje antes de enviar`,`#ef4444`);return}let n=e.querySelector(`input[name="comm-channel"]:checked`)?.value||`inapp`,r=o?.value?o.options[o.selectedIndex].text:`Mensaje del equipo Amaru`,i=[],c=n===`email`||n===`both`;c&&a.forEach(e=>{e.email&&e.email.includes(`@`)&&!e.email.includes(`@amaru.local`)&&i.push(e.email)});try{window.showToast(`Enviando a ${a.length} socios vía ${n}...`,`#f59e0b`);let e=0,o=0;if(n===`inapp`||n===`both`){a.map(e=>e.id);let n=a.map(e=>({user_id:e.id,title:r,message:t.replace(/{nombre}/g,e.full_name||`Atleta`),type:`mass`,sender_id:window.appState?.user?.uid||null}));try{let{error:t}=await window.supabase.from(`user_notifications`).insert(n);if(t)throw t;e=a.length}catch(e){console.error(`[Comunicaciones] Fallo masivo In-App:`,e)}}if(c&&i.length>0)try{let e=await window.SupabaseService.sendBulkEmail(i,r,t.replace(/{nombre}/g,`Atleta`),null);o=e?.totalSent||0,e?.totalFailed>0&&console.warn(`[Comunicaciones] Emails fallados:`,e.failed)}catch(e){console.error(`[Comunicaciones] Error enviando emails:`,e)}let l=``;l=n===`both`?`${e} In-App + ${o} Email ✅`:n===`email`?`${o} emails enviados ✅`:`${e} notificaciones enviadas ✅`,window.showToast(l,`#22c55e`),s.value=``}catch(e){console.error(`[Comunicaciones] Error enviando:`,e),window.showToast(`Error al enviar notificaciones. Intenta de nuevo.`,`#ef4444`)}})},ve=(e,t,n,r)=>{let i=e.membership_expiry?new Date(e.membership_expiry):null,a=i?i-t:null,o=i?Math.ceil(a/(1e3*60*60*24)):null,s=F.filter(t=>t.user_id===e.id);s.sort((e,t)=>new Date(t.reservation_date)-new Date(e.reservation_date));let c=s.length>0?new Date(s[0].reservation_date):null,l=c?Math.floor((t-c)/(1e3*60*60*24)):-1,u=s.filter(e=>{let t=new Date(e.reservation_date);return t.getMonth()===n&&t.getFullYear()===r}).length,d=`Activo`,f=`#22c55e`,p=`rgba(34, 197, 94, 0.15)`;return e.is_frozen?(d=`Congelado`,f=`#3b82f6`,p=`rgba(59, 130, 246, 0.15)`):e.membership_status===`inactive`?(d=`Inactivo`,f=`#ef4444`,p=`rgba(239, 68, 68, 0.15)`):o!==null&&o<=0?o>-30?(d=`Moroso`,f=`#f97316`,p=`rgba(249, 115, 22, 0.15)`):(d=`Inactivo`,f=`#ef4444`,p=`rgba(239, 68, 68, 0.15)`):o===null&&e.membership_status!==`active`&&(d=`Inactivo`,f=`#ef4444`,p=`rgba(239, 68, 68, 0.15)`),{...e,_status:d,_statusColor:f,_statusBg:p,_expiryDate:i,_daysLeft:o,_lastAttendance:c,_daysSinceLastAttendance:l,_monthRes:u,_totalRes:s.length}},ye=async e=>{let t=P.find(t=>t.id===e);if(!t||!t.membership_plan_id){window.showToast(`El socio no tiene un plan asignado para renovar.`,`#ef4444`);return}if(confirm(`¿Renovar el plan de ${t.full_name} por 1 mes más?`))try{let t=new Date;t.setMonth(t.getMonth()+1),await window.supabase.from(`profiles`).update({membership_expiry:t.toISOString(),membership_status:`active`,is_frozen:!1}).eq(`id`,e),window.showToast(`Plan renovado exitosamente ✅`,`#22c55e`),G()}catch(e){console.error(e),window.showToast(`Error al renovar plan.`,`#ef4444`)}},q=window.Swal,J=()=>document.getElementById(`admin-content-area`),be=async()=>{J().innerHTML=`
        <div class="glass-premium p-20" id="classes-skeleton">
            ...
            </div>`,window.lucide.createIcons();let s=document.getElementById(`class-search-input`);s&&(s.oninput=()=>{let e=s.value.toLowerCase();document.querySelectorAll(`.class-card-item`).forEach(t=>{let n=t.getAttribute(`data-name`),r=t.getAttribute(`data-type`);t.style.display=n.includes(e)||r.includes(e)?``:`none`})}),document.getElementById(`btn-add-class-admin`).onclick=()=>Ne(),document.querySelectorAll(`.edit-class-btn`).forEach(t=>{t.onclick=n=>{n.stopPropagation();let r=t.getAttribute(`data-id`);e.find(e=>e.id==r)?Ne(r,e):window.showToast(`Clase no encontrada`,`#ef4444`)}}),document.querySelectorAll(`.delete-class-btn`).forEach(e=>{e.onclick=t=>{t.stopPropagation(),je(e.getAttribute(`data-id`))}})}catch(e){console.error(e),J().innerHTML=`
        ...
        <div class="glass-premium p-20" id="attendance-skeleton">
            ...
            </div>`;let i=document.getElementById(`btn-manual-attendance`);i&&(i.onclick=async()=>{try{let e=await o.getAllProfiles(),t=await o.getClasses(),n=e.map(e=>`<option value="${e.id}">${e.full_name||e.email}</option>`).join(``),r=t.map(e=>`<option value="${e.id}">${e.name} (${e.type})</option>`).join(``),{value:i}=await q.fire({title:`Ingreso Manual`,html:`
                    ...
                    ${n.map((e,t)=>{let n=i.filter(t=>t.active_promo===e.code),r=e.expiresAt&&new Date(e.expiresAt)<new Date,a=e.expiresAt?Math.ceil((new Date(e.expiresAt)-new Date)/(1e3*60*60*24)):null,o=r?`#ef4444`:a&&a<=7?`#f59e0b`:`#22c55e`,s=r?`EXPIRADO`:a&&a<=7?`Vence en ${a}d`:`ACTIVO`;return`
                        <div class="discount-card-item" data-code="${e.code.toLowerCase()}"
                            ...
                            <div style="display:flex; gap:8px;">
                                ...
                                <button class="btn-glass-small btn-edit-promo" data-id="${e.id}" style="width:40px; justify-content:center; color:#60a5fa;" title="Editar código">
                                    <i data-lucide="edit-2" style="width:14px;"></i>
                                </button>
                                <button class="btn-glass-small delete btn-delete-promo" data-id="${e.id}" style="width:40px; justify-content:center;" title="Eliminar código">
                                    <i data-lucide="trash-2" style="width:14px;"></i>
                                </button>
                            </div>
                        </div>`}).join(``)}
                </div>
            </div>`,window.lucide.createIcons();let l=document.getElementById(`discount-search-input`);l&&(l.oninput=()=>{let e=l.value.toLowerCase();document.querySelectorAll(`.discount-card-item`).forEach(t=>{t.style.display=t.getAttribute(`data-code`).includes(e)?``:`none`})});let u=null;document.querySelectorAll(`.btn-edit-promo`).forEach(e=>{e.onclick=t=>{t.stopPropagation();let r=e.getAttribute(`data-id`),i=n.find(e=>e.id===r);if(!i)return;if(u=r,document.getElementById(`new-promo-code`).value=i.code,document.getElementById(`new-promo-perc`).value=i.percent,i.expiresAt)try{document.getElementById(`new-promo-exp`).value=new Date(i.expiresAt).toISOString().split(`T`)[0]}catch{document.getElementById(`new-promo-exp`).value=``}else document.getElementById(`new-promo-exp`).value=``;let a=Array.isArray(i.plans)?i.plans:[];document.querySelectorAll(`.promo-plan-checkbox`).forEach(e=>{e.checked=a.length===0||a.includes(e.value)});let o=document.getElementById(`btn-create-promo`);o.textContent=`Actualizar Código`,o.style.background=`var(--accent-emerald, #22c55e)`,document.getElementById(`new-promo-code`).scrollIntoView({behavior:`smooth`})}}),document.getElementById(`btn-create-promo`).onclick=async()=>{let e=document.getElementById(`new-promo-code`).value.trim().toUpperCase(),t=parseInt(document.getElementById(`new-promo-perc`).value),r=document.getElementById(`new-promo-exp`).value,i=document.querySelectorAll(`.promo-plan-checkbox:checked`),a=document.querySelectorAll(`.promo-plan-checkbox`).length,o=Array.from(i).map(e=>e.value);if(!e||isNaN(t)||t<=0||t>100)return window.showToast(`Código o porcentaje inválido`,`#ef4444`);let s={code:e,percent:t,plans:o.length>0&&o.length<a?o:null,expiresAt:r?new Date(r+`T23:59:59`).toISOString():null};try{if(u){let{error:t}=await window.supabase.from(`discounts`).update(s).eq(`id`,u);if(t)throw t;window.showToast(`Código ${e} actualizado ✅`,`#22c55e`)}else{if(n.find(t=>t.code===e))return window.showToast(`Ese código ya existe`,`#ef4444`);let{error:t}=await window.supabase.from(`discounts`).insert(s);if(t)throw t;window.showToast(`Código ${e} creado ✅`,`#22c55e`)}u=null,Te()}catch(e){console.error(e),window.showToast(`Error al guardar código`,`#ef4444`)}},document.querySelectorAll(`.btn-delete-promo`).forEach(e=>{e.onclick=async t=>{if(t.stopPropagation(),confirm(`¿Eliminar este código? Los usuarios perderán el descuento.`))try{let{error:t}=await window.supabase.from(`discounts`).delete().eq(`id`,e.getAttribute(`data-id`));if(t)throw t;window.showToast(`Código eliminado`),Te()}catch{window.showToast(`Error al eliminar`,`#ef4444`)}}})}catch(e){console.error(e),J().innerHTML=`
            <div class="glass-premium p-20" style="text-align:center;">
                <i data-lucide="alert-circle" style="width:48px; height:48px; color:#ef4444; margin-bottom:15px;"></i>
        ...
        z-index: 9999;
        pointer-events: none;
    `,document.body.appendChild(n),window.showToast=(e,t=`var(--accent-purple)`)=>{let r=document.createElement(`div`);r.style.cssText=`
            ...
            align-items: center;
            gap: 12px;
        `,r.innerHTML=`<span>${e}</span>`,n.appendChild(r),requestAnimationFrame(()=>{r.style.transform=`translateX(0)`}),setTimeout(()=>{r.style.transform=`translateX(120%)`,r.style.opacity=`0`,setTimeout(()=>r.remove(),500)},4e3)},window.showLoading=(e=`Cargando...`)=>{typeof Swal<`u`&&Swal.fire({title:e,allowOutsideClick:!1,background:`#1f1f2e`,color:`#fff`,didOpen:()=>{Swal.showLoading();let e=Swal.getPopup().querySelector(`.swal2-loader`);e&&(e.style.borderColor=`var(--accent-cyan, #00f0ff) transparent var(--accent-cyan, #00f0ff) transparent`)}})},window.hideLoading=()=>{typeof Swal<`u`&&Swal.close()};let r=new URLSearchParams(window.location.search);r.get(`payment`)===`success`?(showToast(`¡Pago procesado con éxito! Bienvenido 🥋`,`#22c55e`),window.history.replaceState({},document.title,window.location.pathname)):r.get(`payment`)===`failure`&&(showToast(`El pago no pudo completarse. Intenta de nuevo. ❌`,`#ef4444`),window.history.replaceState({},document.title,window.location.pathname)),Object.assign(c,{attendance:0,attendanceGoal:4,membershipLimit:2,reservations:[],tournaments:[],classes:[],plans:[],isAdminMode:localStorage.getItem(`isAdminMode`)===`true`,activeFilter:`Todas`,selectedDate:new Date().toISOString().split(`T`)[0],role:`athlete`,level:0,xp:0,notifications:[],membershipExpiry:`2026-02-28`,attendanceHistoryCount:0,surchargePct:30,unsubscribeReservations:null,activePromo:null,proRataPreference:null}),window.appState=c,window.SupabaseService=o;let i=[`El único entrenamiento malo es el que no sucedió.`,`No entrenas para ser mejor que otros, entrenas para que tu 'yo' de ayer no pueda alcanzarte.`,`La disciplina es el puente entre la intención y el cinturón negro.`,`El sudor es la tinta con la que escribes tu propia historia de superación.`,`En el dojo, el ego es el primer oponente que debes derribar antes de saludar al maestro.`,`La mente domina, el cuerpo obedece; si la mente no se rinde, el cuerpo es invencible.`,`La calma en el combate no nace de la falta de miedo, sino del exceso de preparación.`,`Caer siete veces y levantarse ocho no es solo una técnica de Jiu Jitsu/Judo/Wrestling, es una filosofía de vida.`,`El dolor del entrenamiento es temporal, pero el orgullo de la victoria sobre ti mismo es eterno.`,`No busques una vida fácil, busca la fortaleza mental para superar una difícil.`,`Tu mayor rival no está frente a ti con guantes, está dentro de ti pidiendo que te detengas. No lo escuches.`],a=()=>{let e=document.getElementById(`motivational-quote`);e&&(e.innerText=`"${i[Math.floor(Math.random()*i.length)]}"`)},f=()=>{let e=document.getElementById(`nav-admin`),t=document.getElementById(`btn-back-admin`),n=document.getElementById(`btn-dashboard-back-admin`);c.role===`admin`?c.isAdminMode?(e&&e.classList.remove(`hidden`),t&&t.classList.remove(`hidden`),n&&n.classList.remove(`hidden`)):(e&&e.classList.add(`hidden`),t&&t.classList.remove(`hidden`),n&&n.classList.remove(`hidden`)):(e&&e.classList.add(`hidden`),t&&t.classList.add(`hidden`),n&&n.classList.add(`hidden`))},p=e=>{`${e}`,c.isAdminMode=e,localStorage.setItem(`isAdminMode`,e),f(),e?(v(`admin-panel`),showToast(`Modo Administrador Activo 🛡️`,`#ef4444`)):(v(`dashboard`),showToast(`Modo Usuario Activo 👤`))};typeof MercadoPago<`u`&&new MercadoPago(`APP_USR-126c732c-4185-4911-82a0-e7452c2f1243`,{locale:`es-CL`});let m={async createPreference(e){console.log(`Creando Preferencia de Mercado Pago para:`,e.name),showToast(`Conectando con Mercado Pago... 💳`,`#22c55e`);let t=auth.currentUser;if(!t)return showToast(`Debes iniciar sesión`,`#ef4444`);try{let{data:n,error:r}=await window.supabase.functions.invoke(`create-mp-preference`,{body:{plan:e,userId:t.uid,userEmail:t.email}});if(r)throw Error(`Error en el servidor al generar el pago: `+r.message);if(c.activePromo&&await window.supabase.from(`profiles`).update({active_promo:c.activePromo.code}).eq(`id`,t.uid),n&&n.init_point)showToast(`Redirigiendo a entorno seguro... 🔒`,`#22c55e`),window.location.href=n.init_point;else throw Error(`No se pudo obtener el link de pago`)}catch(e){console.error(`Payment creation error:`,e),showToast(`Fallo al conectar con la pasarela de pagos ❌`,`#ef4444`)}}},h=document.querySelectorAll(`.nav-item`),g=document.querySelectorAll(`.screen`),_=e=>{e&&(e.classList.remove(`screen-appear`),e.offsetWidth,e.classList.add(`screen-appear`))},v=e=>{`${e}`,h.forEach(t=>{t.getAttribute(`data-screen`)===e?t.classList.add(`active`):t.classList.remove(`active`)}),g.forEach(e=>{e.classList.remove(`active`),e.classList.remove(`screen-appear`)});let t=document.getElementById(e);t&&(t.classList.add(`active`),_(t));let n=document.getElementById(`bottom-nav`);n&&(n.style.transform=e===`dashboard`?`translateY(100%)`:`translateY(0)`,n.style.transition=`transform 0.3s cubic-bezier(0.4, 0, 0.2, 1)`),e===`dashboard`&&(M(),P(),ce(),a(),R(),z(),V()),e===`schedule`&&Q(),e===`tournaments`&&$(),e===`notifications`&&le(),e===`profile`&&(b(),ue())},y=e=>{if(c.attendanceHistoryCount=e.length,!e||e.length===0){c.currentMonthAttendance=0,c.currentStreak=0;return}let t=new Date,n=t.getMonth(),r=t.getFullYear();c.currentMonthAttendance=e.filter(e=>{let t=new Date(e.attended_at);return t.getMonth()===n&&t.getFullYear()===r}).length;let i=[...new Set(e.map(e=>new Date(e.attended_at).toISOString().split(`T`)[0]))].sort().reverse(),a=0,o=new Date,s=o.toISOString().split(`T`)[0];if(i.includes(s))a++,o.setDate(o.getDate()-1);else{let e=new Date;e.setDate(e.getDate()-1);let t=e.toISOString().split(`T`)[0];i.includes(t)&&(a++,o.setDate(o.getDate()-2))}if(a>0)for(;;){let e=o.toISOString().split(`T`)[0];if(i.includes(e))a++,o.setDate(o.getDate()-1);else break}c.currentStreak=a},b=async()=>{let e=auth.currentUser;if(e)try{let t=await o.getAttendance(e.uid);y(t);let n=new Date,r=[0,0,0,0];t.forEach(e=>{let t=new Date(e.attended_at),i=Math.floor((n-t)/(1e3*60*60*24)),a=Math.floor(i/7);a>=0&&a<4&&r[3-a]++}),I(),oe(r)}catch(e){console.error(`Error updating profile stats:`,e)}},x=0,S=null;h.forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-screen`);if(c.role===`admin`)if(t===`dashboard`){if(x++,`${x}`,clearTimeout(S),x>=3){p(!0),x=0;return}S=setTimeout(()=>{x=0},1e3)}else x=0;if(t===`admin-panel`)if(c.role===`admin`)p(!0);else{showToast(`Acceso restringido 🔒`,`#ef4444`);return}v(t)})});let C=!1,w=!1,ee=(e=`CARGANDO DATOS...`,t=!1,n=``)=>{let r=document.getElementById(`auth-loading-overlay`);r||(r=document.createElement(`div`),r.id=`auth-loading-overlay`,r.style.cssText=`
            ...
            <div style="display:flex; flex-direction:column; align-items:center; max-width: 320px; width: 100%;">
                ...
                @keyframes spin { from { transform: rotate(0deg); } to { transform: rotate(360deg); } }
            ...
            </div>`,r.style.opacity=`1`,r.style.pointerEvents=`auto`;let i=document.getElementById(`btn-auth-retry`);i&&(i.onmouseover=()=>{i.style.background=`var(--accent-purple)`},i.onmouseout=()=>{i.style.background=`rgba(139,92,246,0.2)`},i.onclick=()=>{window.supabase.auth.getSession().then(({data:e})=>{O(e.session)})});let a=document.getElementById(`btn-auth-skip`);a&&(a.onmouseover=()=>{a.style.borderColor=`rgba(255,255,255,0.3)`,a.style.color=`white`},a.onmouseout=()=>{a.style.borderColor=`rgba(255,255,255,0.1)`,a.style.color=`var(--text-gray)`},a.onclick=()=>{T(),showToast(`Carga omitida. Algunos datos pueden no estar disponibles.`,`#eab308`)}),window.lucide&&window.lucide.createIcons()},T=()=>{let e=document.getElementById(`auth-loading-overlay`);e&&(e.style.opacity=`0`,e.style.pointerEvents=`none`,setTimeout(()=>{e.remove()},400))},te=(e,t)=>{c._channels&&c._channels.forEach(e=>{try{window.supabase.removeChannel(e)}catch{}}),c._channels=[];let n=document.getElementById(`user-combat-style`),r=window.supabase.channel(`profile:${e.uid}`).on(`postgres_changes`,{event:`UPDATE`,filter:`id=eq.${e.uid}`,schema:`public`,table:`profiles`},e=>{let r=e.new;Object.assign(t,r),c.level=r.level,c.xp=r.xp,c.membershipLimit=r.membership_limit,c.membershipStatus=r.membership_status||`inactive`,n&&r.combat_style&&(n.value=r.combat_style),I(),f()}).subscribe();c._channels.push(r);let i=window.supabase.channel(`global-data`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`classes`},async()=>{try{c.classes=await o.getClasses(),Q()}catch(e){console.error(`Realtime classes error:`,e)}}).on(`postgres_changes`,{event:`*`,schema:`public`,table:`membership_plans`},async()=>{try{c.plans=await o.getPlans()}catch(e){console.error(`Realtime plans error:`,e)}}).on(`postgres_changes`,{event:`*`,schema:`public`,table:`tournaments`,filter:`user_id=eq.${e.uid}`},async()=>{try{c.tournaments=await o.getTournaments(e.uid),$(),ce()}catch(e){console.error(`Realtime tournaments error:`,e)}}).subscribe();if(c._channels.push(i),c.role===`admin`){let e=window.supabase.channel(`admin-payments`).on(`postgres_changes`,{event:`*`,schema:`public`,table:`payments`},async()=>{let e=document.querySelector(`#admin-content-area h3`)?.innerText;e&&(e.includes(`Validación de Pagos`)||e.includes(`Pagos`))&&ne(),showToast(`¡Actualización de pago detectada! 💳`,`#22c55e`)}).subscribe();c._channels.push(e)}c.unsubscribeReservations&&c.unsubscribeReservations();let a=window.supabase.channel(`reservations:${e.uid}`).on(`postgres_changes`,{event:`*`,filter:`user_id=eq.${e.uid}`,schema:`public`,table:`reservations`},async()=>{try{c.reservations=(await o.getReservations(c.selectedDate)).filter(t=>t.user_id===e.uid).map(e=>e.class_id),c.allUserReservations=await o.getUserReservations(e.uid),c.xp=(t.xp||0)+(c.allUserReservations?.length||0)*50,I(),Q(),M()}catch(e){console.error(`Realtime reservations error:`,e)}}).subscribe();c.unsubscribeReservations=()=>{window.supabase.removeChannel(a)},c._channels.push(a)},E=async(e,t)=>{try{y(await l(o.getAttendance(e.uid),8e3,`Asistencia`)||[]),M()}catch(e){console.warn(`[Background] Error loading attendance:`,e.message)}try{c.role===`admin`?c.notifications=await l(o.getNotifications(),8e3,`Notificaciones admin`):c.notifications=(await l(o.getNotifications(),8e3,`Notificaciones`)).filter(e=>e.is_active!==!1)}catch(e){console.warn(`[Background] Error loading notifications:`,e.message)}try{c.userNotifications=await l(o.getUserNotifications(e.uid),8e3,`Notificaciones personales`),W()}catch(e){console.warn(`[Background] Error loading user notifications:`,e.message)}try{let[t,n,r,i]=await Promise.all([l(o.getClasses(),8e3,`Clases`),l(o.getPlans(),8e3,`Planes`),l(o.getTournaments(e.uid),8e3,`Torneos`),l(o.getUserReservations(e.uid),8e3,`Reservas`)]);t&&(c.classes=t),n&&(c.plans=n,window.renderPaymentTabs&&window.renderPaymentTabs()),r&&(c.tournaments=r),i&&(c.allUserReservations=i,c.xp=(c.xp||0)+i.length*50),Q(),$(),P(),M()}catch(e){console.warn(`[Background] Error fetching initial data:`,e.message),showToast(`Algunos datos no cargaron. Intenta refrescar. 🔄`,`#eab308`)}try{c.reservations=(await l(o.getReservations(c.selectedDate),8e3,`Reservas iniciales`)).filter(t=>t.user_id===e.uid).map(e=>e.class_id),Q(),M()}catch(e){console.warn(`[Background] Error loading initial reservations:`,e.message)}te(e,t)},D=null,O=async e=>{if(console.log(`[Auth] handleAuthSession called. Session exists:`,!!e,`User exists:`,!!e?.user),w){console.log(`[Auth] handleAuthSession already running, skipping concurrent call`);return}w=!0;try{T(),D&&=(clearTimeout(D),null);let t=e?.user,n=null;t&&(n={...t,uid:t.id,email:t.email});let r=document.getElementById(`auth-screen`),i=document.getElementById(`app-container`);if(!n){console.log(`[Auth] No user in session, showing auth screen`),r.classList.remove(`hidden`),_(r),i.classList.add(`hidden`),C=!1,c.user=null;return}let a=(e,t)=>{c.userProfile=t,c.role=t.role||`athlete`,c.isAdminMode=c.role===`admin`,c.level=t.level||0,c.xp=t.xp||0,c.membershipLimit=t.membership_limit||2,c.membershipStatus=t.membership_status||`inactive`,c.planTheme=t.membership_plans?.theme||`bronze`,c.plan=t.membership_plan_id,c.photoURL=t.photo_url||`../images/icon-192.png`;let n=document.querySelector(`.greeting`);n&&(n.textContent=`¡Hola, ${t.full_name||e.displayName||`Atleta`}!`);let a=document.getElementById(`profile-plan-name`),s=document.getElementById(`profile-plan-status`),l=document.getElementById(`profile-plan-remaining`),u=document.getElementById(`profile-plan-progress`);if(a&&(a.textContent=t.membership_plans?.name||(t.membership_status===`active`?`PLAN ACTIVO`:`SIN PLAN`),t.membership_expiry)){let e=new Date(t.membership_expiry),n=new Date;if(e>n){s.textContent=`ACTIVO`,s.style.background=`#22c55e`;let t=Math.ceil((e-n)/(1e3*60*60*24));l.textContent=`${t} Días restantes`;let r=Math.max(0,Math.min(100,(30-t)/30*100));u&&(u.style.width=`${r}%`)}else s.textContent=`INACTIVO`,s.style.background=`#ef4444`,l.textContent=`Renovación requerida`}I();let d=document.getElementById(`user-combat-style`);d&&(d.value=t.combat_style||`striker`,d.onchange=async t=>{let n=t.target.value;window.showLoading(`Guardando estilo...`);try{await o.updateProfile(e.uid,{combat_style:n}),window.hideLoading(),showToast(`¡Estilo actualizado!`,`#22c55e`)}catch(e){console.error(e),showToast(`Error al guardar estilo`,`#ef4444`)}});let m=document.getElementById(`nav-admin`),h=document.querySelectorAll(`.nav-item:not(#nav-admin)`);c.role===`admin`?h.forEach(e=>e.classList.remove(`hidden`)):m&&m.classList.add(`hidden`),f(),se();let g=document.getElementById(`btn-back-admin`),y=document.getElementById(`btn-dashboard-back-admin`);if(g&&(g.onclick=()=>p(!0)),y&&(y.onclick=()=>p(!0)),c.role!==`admin`&&t.membership_status!==`active`){i.classList.add(`hidden`),G();let e=new Date().getDate();if(!c.proRataPreference&&e>=15){let e=document.getElementById(`pro-rata-info-screen`),t=document.getElementById(`membership-selection-screen`);e&&(e.classList.remove(`hidden`),_(e)),t&&t.classList.add(`hidden`)}else{document.getElementById(`pro-rata-info-screen`)?.classList.add(`hidden`);let e=document.getElementById(`membership-selection-screen`);e&&(e.classList.remove(`hidden`),_(e))}}else if(r.classList.add(`hidden`),i.classList.remove(`hidden`),window.location.hash===`#payments`)v(`profile`),document.getElementById(`payment-modal`)?.classList.add(`active`);else if(v(c.isAdminMode?`admin-panel`:`dashboard`),c.isAdminMode){let e=document.getElementById(`admin-content-area`);e&&(!e.innerHTML||e.innerHTML.trim()===``)&&(e.innerHTML=`<div class="p-20 text-center glass" style="border-radius: 12px; margin-top: 20px;">
                            ...
                            </div>`,window.lucide&&window.lucide.createIcons())}let b=document.getElementById(`profile-user-name`),x=document.getElementById(`nav-user-name`),S=e.displayName||(c.role===`admin`?`Administrador`:`Atleta`);b&&(b.innerText=S),x&&(x.innerText=S);let C=document.getElementById(`profile-avatar`);C&&(C.src=c.photoURL||`../images/icon-192.png`)},s=null;try{let e=localStorage.getItem(`amaru_profile_${n.uid}`);e&&(s=JSON.parse(e))}catch(e){console.warn(`[Auth] Error parsing cached profile:`,e)}if(s){console.log(`[Auth] Instant optimistic profile loaded for:`,n.email),a(n,s),T(),(async()=>{try{let e=await l(o.getProfile(n.uid),4e3,`Background Profile`);e?(localStorage.setItem(`amaru_profile_${n.uid}`,JSON.stringify(e)),a(n,e),E(n,e)):E(n,s)}catch(e){console.warn(`[Auth] Background sync completed with cache fallback:`,e.message),E(n,s)}U(n.uid).catch(()=>{})})();return}console.log(`[Auth] First-time login on device. Fetching profile from Supabase...`),ee(`CARGANDO TU PERFIL...`),D=setTimeout(()=>{console.warn(`[Auth] Safety timeout triggered — forcing overlay hide`),T()},1e4);let m=null;try{if(m=await u(()=>l(o.getProfile(n.uid),5e3,`Perfil`),{maxRetries:1,delayMs:600,context:`Cargar perfil`}),m||=(await o.createProfile(n),await l(o.getProfile(n.uid),4e3,`Perfil recién creado`)),m)try{localStorage.setItem(`amaru_profile_${n.uid}`,JSON.stringify(m))}catch{}if(sessionStorage.getItem(`mp_payment_success`)===`true`){sessionStorage.removeItem(`mp_payment_success`);try{let e=(await o.getPayments(n.uid)).find(e=>e.status===`pending`);if(e){await o.updatePaymentStatus(e.id,`approved`,`mercadopago`),showToast(`Pago validado automáticamente ✨`,`#22c55e`);let t=new Date;t.setMonth(t.getMonth()+1),await o.updateProfile(n.uid,{membership_status:`active`,membership_expiry:t.toISOString().split(`T`)[0]}),m=await o.getProfile(n.uid),m&&localStorage.setItem(`amaru_profile_${n.uid}`,JSON.stringify(m))}}catch(e){console.error(`Error confirmando pago:`,e)}}a(n,m),T(),D&&=(clearTimeout(D),null),E(n,m),U(n.uid).catch(()=>{})}catch(e){console.error(`[Auth] Error during profile load:`,e),ee(`Error al cargar`,!0,d(e)?`Verifica tu conexión a internet e intenta de nuevo.`:e.message||`Error al iniciar sesión.`);return}}finally{w=!1}};window.supabase.auth.onAuthStateChange(async(e,t)=>{if(e===`SIGNED_IN`){C=!0,await O(t);return}e===`INITIAL_SESSION`&&C||(e===`INITIAL_SESSION`&&(C=!0),await O(t))}),(async()=>{try{let{data:e,error:n}=await t.auth.getSession();if(n){console.warn(`[Auth] getSession warning:`,n.message);return}e?.session&&!C&&(C=!0,await O(e.session))}catch(e){console.error(`[Auth] Error getting initial session:`,e)}})(),s(v,G);try{Ge!==void 0&&Ge.init(),Ke!==void 0&&Ke.init(),qe!==void 0&&qe.init(),Ye!==void 0&&Ye.init(),console.log(`[INIT] All enhancement modules initialized`)}catch(e){console.error(`[INIT] Error initializing enhancement modules:`,e)}let k=document.getElementById(`btn-edit-avatar`),A=document.getElementById(`btn-delete-avatar`),re=document.getElementById(`avatar-input`);k&&re&&(k.onclick=()=>re.click(),re.onchange=async e=>{let t=e.target.files[0];if(!t)return;let n=auth.currentUser;if(n){showToast(`Subiendo imagen... 📸`);try{let e=await o.uploadAvatar(n.uid,t);await n.updateProfile({photoURL:e}),await o.updateProfile(n.uid,{photo_url:e});let r=document.getElementById(`profile-avatar`);r&&(r.src=e),showToast(`Foto de perfil actualizada ✨`,`#22c55e`)}catch(e){console.error(e),showToast(`Error al subir foto ❌`,`#ef4444`)}}}),A&&(A.onclick=async()=>{let e=auth.currentUser;if(e){showToast(`Eliminando foto... 🗑️`);try{await e.updateProfile({photoURL:``}),await o.updateProfile(e.uid,{photo_url:null});let t=document.getElementById(`profile-avatar`);t&&(t.src=`../images/icon-192.png`),showToast(`Foto eliminada ✅`)}catch(e){console.error(e),showToast(`Error al eliminar ❌`,`#ef4444`)}}});let ie=document.getElementById(`btn-exit-admin-mode`);ie&&(ie.onclick=()=>p(!1));let ae=null,oe=(e=[0,0,0,0])=>{let t=document.getElementById(`attendanceChart`);if(!t)return;ae&&ae.destroy();let n=e.every(e=>e===0)?[0,0,0,0]:e;ae=new Chart(t,{type:`line`,data:{labels:[`Sem 1`,`Sem 2`,`Sem 3`,`Sem 4`],datasets:[{label:`Clases`,data:n,borderColor:`#CBF2F0`,tension:.4,fill:!0,backgroundColor:`rgba(203, 242, 240, 0.1)`}]},options:{responsive:!0,maintainAspectRatio:!1,plugins:{legend:{display:!1}},scales:{y:{display:!1,beginAtZero:!0},x:{grid:{display:!1}}}}})},se=()=>{let e=document.querySelector(`.date-carousel-premium`);if(!e)return;let t=[`DOM`,`LUN`,`MAR`,`MIÉ`,`JUE`,`VIE`,`SÁB`],n=new Date,r=``;for(let e=0;e<7;e++){let i=new Date;i.setDate(n.getDate()+e);let a=i.toISOString().split(`T`)[0],o=a===c.selectedDate;r+=`
                <div class="date-chip ${o?`active`:``}" data-date="${a}">
                    <span>${t[i.getDay()]}</span>
                    <p>${i.getDate()}</p>
                </div>
            `}e.innerHTML=r,e.querySelectorAll(`.date-chip`).forEach(e=>{e.onclick=async()=>{c.selectedDate=e.getAttribute(`data-date`),se();let t=auth.currentUser;t&&(c.reservations=(await o.getReservations(c.selectedDate)).filter(e=>e.user_id===t.uid).map(e=>e.class_id),Q())}})},j=!1;window.toggleReservation=async e=>{if(j)return;let t=auth.currentUser;if(!t){showToast(`Inicia sesión 🔒`,`#ef4444`);return}let n=new Date,r=n.getFullYear()+`-`+String(n.getMonth()+1).padStart(2,`0`)+`-`+String(n.getDate()).padStart(2,`0`);if(c.role!==`admin`&&c.selectedDate!==r){showToast(`Solo puedes agendar clases para el día de hoy 📅`,`#ef4444`);return}let i=c.classes.find(t=>t.id===e);if(!i)return;let a=c.reservations.includes(e);j=!0;try{if(a){let n=new Date,r=(new Date(`${c.selectedDate}T${i.time}`)-n)/(1e3*60*60);if(c.role!==`admin`&&r<1){showToast(`No puedes cancelar a menos de 1 hr ⏳`,`#ef4444`);return}await o.deleteReservation(t.uid,e,c.selectedDate),c.reservations=c.reservations.filter(t=>t!==e),c.allUserReservations&&=c.allUserReservations.filter(t=>!(t.class_id===e&&t.reservation_date===c.selectedDate)),setTimeout(()=>{Q(),M(),P()},0),showToast(`Reserva cancelada 🗓️`,`#71717A`)}else{if(c.membershipStatus!==`active`&&c.role!==`admin`){showToast(`Membresía inactiva. ¡Actívala ahora! 🔒`,`#f59e0b`),v(`membership`);return}if(c.plan){let e=c.plans.find(e=>e.name===c.plan||e.id===c.plan);if(e&&e.days&&e.days.length>0){let t=new Date(c.selectedDate+`T12:00:00`).getDay();if(!e.days.includes(t))return showToast(`Plan no válido para este día 🗓️`,`#ef4444`)}let t=c.selectedDate.substring(0,7),n=c.allUserReservations?c.allUserReservations.filter(e=>e.reservation_date&&e.reservation_date.startsWith(t)):[],r=999;if(e&&(r=e.monthly||999),n.length>=r){let e=document.getElementById(`upsell-modal`);return e&&(e.classList.remove(`hidden`),lucide.createIcons()),j=!1,showToast(`Límite mensual agotado ⚠️`,`#eab308`)}}if(c.reservations.length>=c.membershipLimit){let e=document.getElementById(`upsell-modal`);return e&&(e.classList.remove(`hidden`),lucide.createIcons()),j=!1,showToast(`Límite diario alcanzado ⚠️`,`#eab308`)}await o.createReservation(t.uid,e,i.name,c.selectedDate),c.reservations.includes(e)||c.reservations.push(e),c.allUserReservations&&c.allUserReservations.push({user_id:t.uid,class_id:e,class_name:i.name,reservation_date:c.selectedDate}),setTimeout(()=>{Q(),M(),P()},0);let n=10,r=(i.type||``).toLowerCase();(r.includes(`gi`)||r.includes(`no-gi`))&&(n=15),r.includes(`open`)&&(n=5),await o.updateProfile(t.uid,{xp:(c.xp||0)+n}),c.xp=(c.xp||0)+n,I(),showToast(`¡Clase reservada con éxito! 🥋`,`#22c55e`)}}catch(e){console.error(`Supabase Reservation Error:`,e),showToast(`Error procesando reserva ❌`,`#ef4444`)}finally{j=!1}},window.renderSchedule=Q,window.renderTournaments=$;let M=()=>{let e=document.getElementById(`attendance-circle`),t=document.getElementById(`attendance-percent`),n=document.getElementById(`completed-classes-count`),r=document.getElementById(`user-streak-text`);if(r&&(r.innerText=`${c.currentStreak||0} Días`),e&&t){let r=c.membershipLimit||5;r>10&&(r=5);let i=r*4,a=c.currentMonthAttendance||0,o=Math.min(100,Math.round(a/i*100)),s=`var(--amaru-gold)`;s=o<50?`#ef4444`:o<100?`#eab308`:`#22c55e`,e.style.stroke=s;let l=188.5-o/100*188.5;if(e.style.strokeDashoffset=l,t.innerText=`${o}%`,n){let e=a,t=0,r=e=>{n.innerHTML=`${e} <span style="font-size: 1rem; color: var(--text-gray); font-weight: 600;">/ ${i}</span>`};if(e>0){let n=Math.max(1,Math.ceil(e/(1500/30))),i=setInterval(()=>{if(t+=n,t>=e){if(t=e,clearInterval(i),r(t),o>=100&&auth.currentUser){let e=`goal_reward_${new Date().getFullYear()}_${new Date().getMonth()}_${auth.currentUser.uid}`;localStorage.getItem(e)||N(e)}}else r(t)},30)}else r(0)}}},N=async e=>{typeof confetti==`function`&&confetti({particleCount:150,spread:80,origin:{y:.6},colors:[`#D4AF37`,`#ffffff`,`#22c55e`]}),showToast(`¡Meta Mensual Alcanzada! +500 XP Extra 🏆`,`var(--amaru-gold)`);try{let t=auth.currentUser;t&&(c.xp=(c.xp||0)+500,await o.updateProfile(t.uid,{xp:c.xp}),I(),localStorage.setItem(e,`true`))}catch(e){console.error(`Error giving month reward`,e)}},P=()=>{let e=document.getElementById(`dynamic-next-class`);if(!e)return;let t=new Date,n=t.toISOString().split(`T`)[0],r=t.getHours()*60+t.getMinutes();t.getDay();let i=null,a=!1;if(c.allUserReservations&&c.allUserReservations.length>0){let e=c.allUserReservations.filter(e=>e.reservation_date>=n).map(e=>{let t=c.classes.find(t=>t.id===e.class_id);if(!t)return null;let[n,r]=t.time.split(`:`).map(Number);return{...e,classData:t,totalMinutes:n*60+r}}).filter(e=>e!==null).sort((e,t)=>e.reservation_date===t.reservation_date?e.totalMinutes-t.totalMinutes:e.reservation_date.localeCompare(t.reservation_date)).find(e=>e.reservation_date>n?!0:e.totalMinutes>r+10);e&&(i=e.classData,a=!0)}if(!i&&c.classes.length>0)for(let e=0;e<7;e++){let n=new Date;n.setDate(t.getDate()+e);let a=n.getDay(),o=c.classes.filter(e=>{let t=e.days;if(typeof t==`string`)try{t=JSON.parse(t)}catch{}return Array.isArray(t)&&t.includes(a)}).sort((e,t)=>{let[n,r]=e.time.split(`:`).map(Number),[i,a]=t.time.split(`:`).map(Number);return n*60+r-(i*60+a)});if(e===0){let e=o.find(e=>{let[t,n]=e.time.split(`:`).map(Number);return t*60+n>r});if(e){i=e;break}}else if(o.length>0){i=o[0];break}}if(i){if(e.classList.remove(`smoke-purple`,`smoke-cyan`,`smoke-gold`,`smoke-crimson`),i.theme&&e.classList.add(i.theme),e.innerHTML=`
            ...
            `,lucide.createIcons(),a){let e=document.getElementById(`btn-checkin-dash`);e&&(e.onclick=()=>F(i))}document.getElementById(`btn-dashboard-go-schedule`).onclick=()=>{let e=document.querySelector(`[data-screen="schedule"]`);e&&e.click()}}else e.innerHTML=`<div class="p-30 text-center opacity-50">No hay clases programadas próximamente</div>`},F=async e=>{let t=auth.currentUser;if(t){showToast(`Registrando asistencia... 🥋`);try{await o.logAttendance(t.uid,e.id,e.name);let n=(c.xp||0)+25;await o.updateProfile(t.uid,{xp:n}),c.xp=n,c.attendanceHistoryCount++,showToast(`¡Asistencia confirmada! +25 XP 🔥`,`#22c55e`),I(),P(),M()}catch(e){console.error(`Check-in error:`,e),showToast(`Ya registraste asistencia hoy o hubo un error 🛡️`,`#eab308`)}}},ce=()=>{let e=document.getElementById(`motivation-container`);if(!e)return;e.innerHTML=``;let t=c.tournaments[0];if(t){let n=new Date(t.date)-new Date,r=Math.floor(n/(1e3*60*60*24));if(r>0){let n=document.createElement(`div`);n.className=`glass-premium tournament-countdown-card`,n.style.margin=`0 20px 20px`,n.style.position=`relative`,n.style.overflow=`hidden`,n.innerHTML=`
                ...
                `,e.prepend(n)}}lucide.createIcons()},I=()=>{let e=document.getElementById(`user-rank-status`),t=c.planTheme===`gold`?100:c.planTheme===`silver`?30:10,n=Math.floor(c.xp||0),r=Math.min(Math.floor(n/100),t),i=Math.min(n%100/100*100,100);r>=t&&(i=100);let a=r<10?`Nomad`:r<30?`Warrior`:r<50?`Elite`:`Legend`;c.level=r;let o={Nomad:`#fbbf24`,Warrior:`#f97316`,Elite:`#a855f7`,Legend:`#22c55e`}[a]||`#fbbf24`;e&&(e.innerText=c.role===`admin`?`Admin`:a,e.style.color=c.role===`admin`?`#ef4444`:o,e.style.background=c.role===`admin`?`rgba(239,68,68,0.15)`:`${o}22`,e.style.borderColor=c.role===`admin`?`rgba(239,68,68,0.3)`:`${o}44`);let s=document.getElementById(`profile-user-level`);s&&(s.innerText=c.role===`admin`?`Administrador`:`Nivel ${r}`);let l=document.getElementById(`profile-xp-text`);l&&(l.innerText=`${n} / ${(r+1)*100} XP`);let u=document.getElementById(`profile-rank-bar`);u&&requestAnimationFrame(()=>{u.style.width=`${i}%`});let d=document.getElementById(`profile-plan-chip`);if(d){let e=c.membershipStatus===`active`||c.role===`admin`;d.innerText=e?`Activo`:`Inactivo`,d.style.background=e?`rgba(34,197,94,0.15)`:`rgba(239,68,68,0.15)`,d.style.color=e?`#22c55e`:`#ef4444`,d.style.borderColor=e?`rgba(34,197,94,0.3)`:`rgba(239,68,68,0.3)`}let f=document.getElementById(`prof-stat-classes`),p=document.getElementById(`prof-stat-streak`),m=document.getElementById(`prof-stat-xp`),h=document.getElementById(`prof-stat-reserv`);f&&(f.innerText=c.attendanceHistoryCount||0),p&&(p.innerText=c.currentStreak||0),m&&(m.innerText=n),h&&(h.innerText=c.allUserReservations?.length||0);let g=document.querySelector(`.rank-bar-fill`),_=document.querySelector(`.rank-info-text`);g&&(g.style.width=`${i}%`),_&&(_.innerHTML=`<span>${n} / ${t*100} XP Max</span><strong>${Math.floor(i)}% Lvl Up</strong>`);let v=document.querySelector(`#profile-user-level span`);v&&(v.innerText=c.role===`admin`?`MAX`:r);let y=document.querySelector(`.lvl-fill`),b=document.querySelector(`.level-indicator span`);y&&(y.style.width=c.role===`admin`?`100%`:`${i}%`),b&&(b.innerText=c.role===`admin`?`LVL MAX`:`NVL ${r}`),H(),B(r,n,t*100,i,a),L()},L=()=>{let e=document.getElementById(`profile-activity-timeline`);if(!e)return;let t=[];if(c.allUserReservations&&c.allUserReservations.length>0&&[...c.allUserReservations].sort((e,t)=>new Date(t.reservation_date)-new Date(e.reservation_date)).slice(0,5).forEach(e=>{let n=new Date(e.reservation_date),r=n.toLocaleDateString(`es-ES`,{weekday:`short`}),i=n.getDate();t.push({type:`reserva`,icon:`calendar-check`,color:`#22c55e`,title:`Reserva: ${e.class_name||`Clase`}`,desc:`${r.charAt(0).toUpperCase()+r.slice(1)} ${i}`,date:n})}),c.level>0&&t.push({type:`nivel`,icon:`zap`,color:`#fbbf24`,title:`¡Nivel ${c.level} alcanzado!`,desc:`Subiste de rango`,date:new Date}),t.sort((e,t)=>t.date-e.date),t.length===0){e.innerHTML=`<div style="padding: 20px; text-align: center; opacity: 0.4;"><p style="font-size: 0.75rem;">Sin actividad reciente</p></div>`;return}e.innerHTML=t.map((e,n)=>`
        ...
        `).join(``),window.lucide&&window.lucide.createIcons()},R=()=>{let e=auth.currentUser;if(!e)return;let t=document.getElementById(`dash-avatar`);t&&(t.src=c.photoURL||`../images/icon-192.png`);let n=document.getElementById(`dash-name`);n&&(n.innerText=e.displayName||`Atleta`);let r=document.getElementById(`dash-plan-chip`);if(r){let e=c.membershipStatus===`active`||c.role===`admin`;r.innerText=e?`Activo`:`Inactivo`,r.style.background=e?`rgba(34,197,94,0.15)`:`rgba(239,68,68,0.15)`,r.style.color=e?`#22c55e`:`#ef4444`,r.style.borderColor=e?`rgba(34,197,94,0.3)`:`rgba(239,68,68,0.3)`}},z=()=>{document.querySelectorAll(`.quick-btn`).forEach(e=>{let t=e.getAttribute(`data-nav`),n=e.getAttribute(`data-action`);e.onmouseenter=()=>{e.style.background=`rgba(255,255,255,0.08)`,e.style.transform=`translateY(-2px)`},e.onmouseleave=()=>{e.style.background=`rgba(255,255,255,0.03)`,e.style.transform=`translateY(0)`},e.onclick=()=>{t&&(v(t),n===`reserve`&&t===`schedule`&&showToast(`Selecciona una clase para reservar 🥋`,`#22c55e`))}})},B=(e,t,n,r,i)=>{let a=document.getElementById(`streak-weekly-dots`);if(a){let e=[`L`,`M`,`M`,`J`,`V`,`S`,`D`],t=new Date,n=t.getDay()===0?6:t.getDay()-1,r=new Date(t);r.setDate(t.getDate()-n),r.setHours(0,0,0,0);let i=new Date(r);i.setDate(r.getDate()+6),i.setHours(23,59,59,999);let o=c.currentStreak||0;a.innerHTML=e.map((e,t)=>{let r=t<=n&&t>=n-Math.min(o-1,n);return`<div style="width: 28px; height: 28px; border-radius: 50%; background: ${r?`rgba(34,197,94,0.2)`:`rgba(255,255,255,0.05)`}; border: ${r?`1px solid rgba(34,197,94,0.4)`:`1px solid rgba(255,255,255,0.08)`}; display: flex; align-items: center; justify-content: center; font-size: 0.55rem; font-weight: 700; color: ${r?`#22c55e`:`var(--text-gray)`}; transition: all 0.3s;">${e}</div>`}).join(``)}let o=document.getElementById(`streak-count-label`);o&&(o.innerText=`${c.currentStreak||0} días seguidos`);let s=document.getElementById(`dash-xp-current`),l=document.getElementById(`dash-xp-target`),u=document.getElementById(`dash-xp-bar`),d=document.getElementById(`dash-xp-percent`);s&&(s.innerText=t),l&&(l.innerText=n),u&&requestAnimationFrame(()=>{u.style.width=`${r}%`}),d&&(d.innerText=`${Math.floor(r)}% para subir`);let f=document.getElementById(`dash-classes-count`),p=document.getElementById(`dash-classes-sparkline`);if(f&&(f.innerText=c.currentMonthAttendance||0),p){let e=[.3,.5,.4,.7];if(c.currentMonthAttendance>0){let t=Math.min(1,c.currentMonthAttendance*.2/5+.2),n=Math.min(1,c.currentMonthAttendance*.3/5+.2),r=Math.min(1,c.currentMonthAttendance*.25/5+.2),i=Math.min(1,c.currentMonthAttendance*.25/5+.2);e[0]=t,e[1]=n,e[2]=r,e[3]=i}p.innerHTML=e.map(e=>`
            ...
            `).join(``)}let m=document.getElementById(`dash-rank-name`),h=document.getElementById(`dash-rank-level`),g=document.querySelector(`#dash-rank-icon i`);if(m){m.innerText=i;let e={Nomad:`#fbbf24`,Warrior:`#f97316`,Elite:`#a855f7`,Legend:`#22c55e`};m.style.color=e[i]||`#fbbf24`}h&&(h.innerText=`Nivel ${e}`),g&&g.setAttribute(`data-lucide`,{Nomad:`shield`,Warrior:`sword`,Elite:`zap`,Legend:`crown`}[i]||`shield`)},V=()=>{let e=document.getElementById(`activity-feed-list`);if(!e)return;let t=[];if(c.allUserReservations&&c.allUserReservations.length>0&&[...c.allUserReservations].sort((e,t)=>new Date(t.reservation_date)-new Date(e.reservation_date)).slice(0,3).forEach(e=>{let n=new Date(e.reservation_date),r=Math.floor((new Date-n)/(1e3*60*60*24)),i=r===0?`Hoy`:r===1?`Ayer`:`Hace ${r} días`;t.push({icon:`calendar-check`,color:`#22c55e`,bg:`rgba(34,197,94,0.1)`,border:`rgba(34,197,94,0.2)`,title:`Reserva confirmada`,desc:`${e.class_name||`Clase`} — ${i}`,time:i})}),c.attendanceHistoryCount>0&&t.push({icon:`check-circle`,color:`#a855f7`,bg:`rgba(168,85,247,0.1)`,border:`rgba(168,85,247,0.2)`,title:`Asistencia registrada`,desc:`+25 XP obtenidos`,time:`Reciente`}),c.level>0&&t.push({icon:`zap`,color:`#fbbf24`,bg:`rgba(251,191,36,0.1)`,border:`rgba(251,191,36,0.2)`,title:`¡Subiste de nivel!`,desc:`Nivel ${c.level} alcanzado`,time:`Reciente`}),c.tournaments&&c.tournaments.length>0){let e=c.tournaments[0],n=Math.ceil((new Date(e.date)-new Date)/(1e3*60*60*24));n>0&&n<=30&&t.push({icon:`trophy`,color:`#f97316`,bg:`rgba(249,115,22,0.1)`,border:`rgba(249,115,22,0.2)`,title:`Torneo próximo`,desc:`${e.name} — ${n} días`,time:`Próximamente`})}c.membershipStatus===`active`&&t.push({icon:`shield-check`,color:`#06b6d4`,bg:`rgba(6,182,212,0.1)`,border:`rgba(6,182,212,0.2)`,title:`Plan activo`,desc:`Membresía al día`,time:`Actual`}),t.length===0?e.innerHTML=`
            ...
            `).join(``),window.lucide&&window.lucide.createIcons()};window.markNotifRead=async e=>{try{if(await o.markNotificationRead(e),c.userNotifications){let t=c.userNotifications.find(t=>t.id===e);t&&(t.is_read=!0)}W(),le()}catch(e){console.error(`[markNotifRead] Error:`,e)}},window.renderActivityFeed=V;let H=()=>{let e=document.getElementById(`user-badges-container`);if(!e)return;let t=[{id:`Novato`,icon:`shield`,unlocked:c.role===`admin`||c.attendanceHistoryCount>=5,desc:`5 Clases tomadas`,msg:`¡El inicio de la grandeza empieza con el primer paso!`},{id:`Constante`,icon:`calendar-check`,unlocked:c.role===`admin`||c.attendanceHistoryCount>=20,desc:`20 Clases tomadas`,msg:`La disciplina es el puente entre metas y logros.`},{id:`Guerrero`,icon:`zap`,unlocked:c.role===`admin`||c.level>=10,desc:`Alcanza Nivel 10`,msg:`Tus rivales tiemblan ante tu poder.`},{id:`Elite`,icon:`crown`,unlocked:c.role===`admin`||c.level>=50,desc:`Alcanza Nivel 50`,msg:`¡Eres una leyenda viviente en el tatami!`}],n=t.filter(e=>e.unlocked).length,r=document.getElementById(`badges-progress-text`);r&&(r.innerText=`${n} / ${t.length}`),e.innerHTML=t.map(e=>`
        ...
        `).join(``),lucide.createIcons()},U=async e=>{try{console.log(`Firebase Messaging removed. Push notifications require Supabase/OneSignal setup.`)}catch(e){console.error(`Error initializing Firebase Messaging:`,e)}},W=()=>{let e=document.getElementById(`nav-notif-badge`),t=(c.userNotifications||[]).filter(e=>!e.is_read).length;e&&(t>0?(e.textContent=t>99?`99+`:t,e.style.display=`flex`):e.style.display=`none`)},le=()=>{let e=document.getElementById(`notifications-list`);if(!e)return;let t=[];if((c.userNotifications||[]).map(e=>{let t=!e.is_read,n=e.type===`mass`?`#f59e0b`:e.type===`direct`?`#8b5cf6`:`var(--accent-cyan)`,r=e.type===`mass`?`Comunicación`:e.type===`direct`?`Mensaje Directo`:`Sistema`,i=e.type===`mass`?`megaphone`:e.type===`direct`?`message-circle`:`bell`,a=new Date(e.created_at),o=new Date-a,s=Math.floor(o/(1e3*60)),c=Math.floor(o/(1e3*60*60)),l=Math.floor(o/(1e3*60*60*24)),u;return u=s<1?`Recién ahora`:s<60?`Hace ${s} min`:c<24?`Hace ${c} h`:l<7?`Hace ${l} d`:a.toLocaleDateString(`es-CL`,{day:`2-digit`,month:`short`}),`
            ...
            `}),c.membershipExpiry){let e=new Date(c.membershipExpiry)-new Date,n=Math.ceil(e/(1e3*60*60*24));if(n<=10&&n>=0){let e=n<=3;t.push(`
            ...
            `);let n=c.notifications.map(e=>{let t=`bell`,n=`var(--accent-purple)`,r=``;return e.type===`welcome`?t=`sparkles`:e.type===`alert`?(t=`alert-triangle`,n=`#ef4444`,r=`<div class="critical-badge" style="background:#ef4444">URGENTE</div>`):e.type===`calendar`&&(t=`calendar`,n=`#f59e0b`,r=`<div class="critical-badge" style="background:#f59e0b">IMPORTANTE</div>`),`
            ...
            `}),r=c.tournaments.map(e=>{let t=Math.ceil((new Date(e.date)-new Date)/(1e3*60*60*24)),n=t<=7;return`
            <div class="notification-item glass ${n?`critical`:``}">
                ...
                ${n?`<div class="critical-badge">EVENTO</div>`:``}
            </div>`});e.innerHTML=[...t,...n,...r].join(``)||`<div class="p-20 text-center opacity-50">No hay notificaciones nuevas.</div>`,lucide.createIcons()},ue=async()=>{let e=document.getElementById(`payment-history-list`),t=document.getElementById(`current-membership-progress`),n=document.getElementById(`btn-view-plans`);if(n&&(n.onclick=()=>{let e=document.getElementById(`membership-selection-screen`);e&&(e.classList.remove(`hidden`),_(e)),typeof G==`function`&&G()}),!(!e||!auth.currentUser)){if(t)try{let e=await o.getProfile(auth.currentUser.uid);if(e&&e.membership_status===`active`){let n=e.membership_plans?.name||`PLAN ACTIVO`;e.membership_plans?.theme;let r=e.membership_plans?.class_limit||e.membership_limit||0,i=0,a=0;if(e.membership_expiry){let t=new Date(e.membership_expiry);a=Math.max(0,Math.ceil((t-new Date)/(1e3*60*60*24)));let n=new Date(t);n.setDate(n.getDate()-30),c.allUserReservations&&(i=c.allUserReservations.filter(e=>{let r=new Date(e.reservation_date);return r>=n&&r<=t}).length)}let o=r>0?Math.min(i/r*100,100):i>0?100:0;t.innerHTML=`
                        ...
                        <div id="membership-extra-details" class="hidden" style="margin-top:20px; padding-top:20px; border-top: 1px solid rgba(255,255,255,0.1); animation: slideInUp 0.5s cubic-bezier(0.4, 0, 0.2, 1);">
                            <div class="details-stats-grid" style="display:grid; grid-template-columns: repeat(2, 1fr); gap:12px; margin-bottom: 20px;">
                                <div class="detail-stat glass" style="padding:15px; text-align:center; border-radius: 18px; border: 1px solid rgba(255,255,255,0.05); background: rgba(255,255,255,0.02); position: relative; overflow: hidden;">
                                    ...
                                    <strong style="font-size:1.6rem; color: white; font-weight: 900;">${c.allUserReservations?.length||0}</strong>
                                ...
                                <div class="detail-stat glass" style="padding:15px; text-align:center; border-radius: 18px; border: 1px solid rgba(255,255,255,0.05); background: rgba(255,255,255,0.02);">
                                    ...
                                    <strong style="font-size:1.6rem; color: #fff; font-weight: 900;">${c.attendanceHistoryCount||0}</strong>
                    ...
                    `;let s=t.querySelector(`#membership-main-info`),l=t.querySelector(`#membership-extra-details`),u=t.querySelector(`#toggle-membership-indicator`);s&&l&&(s.onclick=()=>{l.classList.contains(`hidden`)?(l.classList.remove(`hidden`),u&&(u.style.transform=`rotate(180deg)`),s.style.background=`rgba(255,255,255,0.03)`):(l.classList.add(`hidden`),u&&(u.style.transform=`rotate(0deg)`),s.style.background=`transparent`),lucide.createIcons()});let d=t.querySelector(`#btn-renew-advance-plan`);d&&(d.onclick=()=>{let e=document.getElementById(`membership-selection-screen`);e&&(e.classList.remove(`hidden`),_(e)),typeof G==`function`&&G()}),lucide.createIcons()}else t.innerHTML=`<p style="text-align:center; font-size:0.8rem; color:var(--text-gray);">No tienes una membresía activa actualmente.</p>`}catch(e){console.error(`Error setting membership progress:`,e),t.innerHTML=`<p style="text-align:center; font-size:0.8rem; color:var(--text-gray);">Error al cargar progreso.</p>`}try{let t=await o.getPayments(auth.currentUser.uid),n=document.getElementById(`payments-count`);n&&(n.innerText=`${t.length} pago${t.length===1?``:`s`}`),e.innerHTML=t.length===0?`<div style="padding: 24px; text-align: center; opacity: 0.4;">
                    ...
                    `}).join(``),window.lucide&&window.lucide.createIcons()}catch(e){console.error(`Error rendering payments:`,e)}}};document.getElementById(`admin-content-area`);function G(){let e=document.getElementById(`membership-plans-container`);if(!e)return;let t=c.plans||[];if(t.length===0){e.innerHTML=`<p style="text-align:center; padding:40px; opacity:0.5;">No hay planes disponibles.</p>`;return}let n=t.reduce((e,t)=>t.popular?t:e,t[0]),r={bronze:{accent:`#cd7f32`,icon:`shield`,gradient:`linear-gradient(135deg, rgba(205,127,50,0.15), rgba(205,127,50,0.05))`},silver:{accent:`#c0c0c0`,icon:`shield-check`,gradient:`linear-gradient(135deg, rgba(192,192,192,0.15), rgba(192,192,192,0.05))`},gold:{accent:`#ffd700`,icon:`crown`,gradient:`linear-gradient(135deg, rgba(255,215,0,0.15), rgba(255,215,0,0.05))`}};e.innerHTML=t.map((e,t)=>{let i=e.price,a=``,o=e.subtitle||``,s=`/ MES`,l=``,u=``,d=r[e.theme]||r.bronze,f=e.popular||n&&e.id===n.id;if(c.proRataPreference===`proportional`){let t=new Date,n=t.getDate(),r=new Date(t.getFullYear(),t.getMonth()+1,0).getDate(),u=r-n+1,d=e.price/r,f=1+c.surchargePct/100;i=Math.round(d*u*f),a=`<span style="text-decoration:line-through; opacity:0.4; font-size:0.75em; margin-right:6px;">$${e.price.toLocaleString()}</span>`,o=`Proporcional (${u} días)`,s=``,l=`<span style="font-size:0.6rem; background:rgba(251,191,36,0.15); color:#fbbf24; padding:2px 8px; border-radius:10px; font-weight:700; border:1px solid rgba(251,191,36,0.3);">⚡ Prorrateado</span>`}else if(c.activePromo){let t=!0,n=!1;if(c.activePromo.plans&&c.activePromo.plans.length>0&&(t=c.activePromo.plans.includes(e.id)),c.activePromo.expiresAt){let e=new Date(c.activePromo.expiresAt);e.setDate(e.getDate()+1),n=e<new Date}if(t&&!n){let t=c.activePromo.percent,n=Math.round(e.price*(t/100));i=e.price-n,a=`<span style="text-decoration:line-through; opacity:0.4; font-size:0.75em; margin-right:6px;">$${e.price.toLocaleString()}</span>`,l=`<span style="font-size:0.6rem; background:rgba(34,197,94,0.15); color:#22c55e; padding:2px 8px; border-radius:10px; font-weight:700; border:1px solid rgba(34,197,94,0.3);">🎉 -${t}%</span>`,u=`<span style="font-size:0.7rem; color:#22c55e; font-weight:700;">Ahorras $${n.toLocaleString()}</span>`}}else c.proRataPreference===`full`&&(l=`<span style="font-size:0.6rem; background:rgba(59,130,246,0.15); color:#3b82f6; padding:2px 8px; border-radius:10px; font-weight:700; border:1px solid rgba(59,130,246,0.3);">📅 Mes completo</span>`);let p=Array.isArray(e.features)?e.features:e.features?String(e.features).split(`,`).map(e=>e.trim()).filter(e=>e):[],m=p.length>0?`<div style="display:flex; flex-wrap:wrap; gap:4px; margin:10px 0; justify-content:center;">${p.slice(0,3).map(e=>`<span style="font-size:0.6rem; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); padding:2px 8px; border-radius:20px; color:var(--text-gray);">${e}</span>`).join(``)}${p.length>3?`<span style="font-size:0.6rem; background:rgba(255,255,255,0.05); border:1px solid rgba(255,255,255,0.08); padding:2px 8px; border-radius:20px; color:var(--text-gray);">+${p.length-3}</span>`:``}</div>`:``,h=e.description?`<p style="font-size:0.7rem; color:var(--text-gray); margin:6px 12px 0; line-height:1.4; opacity:0.8; min-height:30px;">${e.description}</p>`:``;return`
        ...
        `}).join(``),lucide.createIcons()}window.togglePlanFlip=e=>{let t=document.getElementById(`plan-card-${e}`);t&&t.classList.toggle(`flipped`)},window.selectMembershipPlan=async e=>{let t=c.plans.find(t=>t.id==e);if(!t)return;let n=auth.currentUser;if(!n)return showToast(`Debes iniciar sesión para continuar`,`#ef4444`);let r={...t};if(c.activePromo){let e=!0;c.activePromo.plans&&c.activePromo.plans.length>0&&(e=c.activePromo.plans.includes(t.id)),e&&(r.price-=r.price*(c.activePromo.percent/100),r.name=`${r.name} (Promo: ${c.activePromo.code})`)}let i=async e=>{try{if(showToast(`Iniciando pago para ${e.name}... 💳`,`#22c55e`),m!==void 0&&typeof m.createPreference==`function`)await m.createPreference(e);else throw Error(`Servicio de pagos no disponible`)}catch(e){console.error(`Auto-payment error:`,e),showToast(`No se pudo iniciar el pago automático. Intenta de nuevo.`,`#ef4444`)}},a=new Date,o=a.getDate(),s=!1;try{let e=await db.collection(`users`).doc(n.uid).get();if(e.exists){let t=e.data();(!t.membership_status||t.membership_status===`pending`)&&(s=!0)}}catch(e){console.error(`Error checking user plan status:`,e)}if(s&&o>=15){let e=new Date(a.getFullYear(),a.getMonth()+1,0).getDate(),t=e-o+1,n=r.price/e,s=1+c.surchargePct/100,l=Math.round(n*t*s),u={...r,price:l,name:`${r.name} (Proporcional resto del mes)`},d={...r,name:`${r.name} (Mes Completo)`};if(c.proRataPreference===`proportional`)return await i(u);if(c.proRataPreference===`full`)return await i(d);if(document.getElementById(`proportional-modal`))document.getElementById(`btn-pay-proportional`).innerHTML=`<strong style="font-size: 1rem;">Pagar Proporcional ($${l.toLocaleString()})</strong><span style="font-size:0.7rem; font-weight:normal; opacity: 0.8; margin-top: 3px;">Lo que resta del mes (incluye recargo)</span>`,document.getElementById(`btn-pay-full`).innerHTML=`<strong style="font-size: 1rem;">Pagar Mes Completo ($${r.price.toLocaleString()})</strong><span style="font-size:0.7rem; font-weight:normal; opacity: 0.8; margin-top: 3px;">Se cobrará el mes entero desde hoy</span>`,document.getElementById(`proportional-modal`).style.display=`flex`;else{let e=`
                ...
                </div>`;document.body.insertAdjacentHTML(`beforeend`,e)}document.getElementById(`btn-pay-proportional`).onclick=async()=>{document.getElementById(`proportional-modal`).style.display=`none`,await i(u)},document.getElementById(`btn-pay-full`).onclick=async()=>{document.getElementById(`proportional-modal`).style.display=`none`,await i(d)},document.getElementById(`btn-cancel-proportional`).onclick=()=>{document.getElementById(`proportional-modal`).style.display=`none`};return}if(c.role===`admin`)return showToast(`El administrador no realiza pagos ⚙️`,`#fbbf24`);await i(r)};let K=document.getElementById(`btn-skip-membership`);K&&(K.onclick=async()=>{document.getElementById(`membership-selection-screen`).classList.add(`hidden`),document.getElementById(`app-container`).classList.remove(`hidden`),v(`dashboard`);let e=auth.currentUser;if(e&&c.role!==`admin`)try{let{data:t}=await window.supabase.from(`payments`).select(`id`).eq(`user_id`,e.uid).eq(`status`,`pending`);(!t||t.length===0)&&await o.recordPayment(e.uid,{amount:0,concept:`Registro - Pago Omitido`,receipt_url:null,status:`pending`,payment_method:`manual`,currency:`COP`})}catch(e){console.error(`Error al registrar el pago omitido:`,e)}showToast(`¡Explora AmaruApp! 🥋`)})}),document.addEventListener(`DOMContentLoaded`,()=>{if(He(),(()=>{let e=document.getElementById(`manage-classes-btn`),t=document.getElementById(`manage-users-btn`),n=document.getElementById(`manage-active-users-btn`),r=document.getElementById(`view-attendance-btn`),a=document.getElementById(`manage-payments-btn`),o=document.getElementById(`view-revenue-btn`),s=document.getElementById(`manage-discounts-btn`),c=document.getElementById(`manage-notifications-btn`);document.getElementById(`btn-export-revenue`),e&&(e.onclick=()=>{let e=document.getElementById(`admin-revenue-section`);e&&e.classList.add(`hidden`),be()}),t&&(t.onclick=()=>Se()),n&&(n.onclick=()=>G()),r&&(r.onclick=()=>we()),a&&(a.onclick=()=>ne()),o&&(o.onclick=()=>i()),s&&(s.onclick=()=>Te()),c&&(c.onclick=()=>Ee())})(),Ge!==void 0&&Ge.init(),Ke!==void 0&&Ke.init(),qe!==void 0&&qe.init(),Ye!==void 0&&Ye.init(),Je!==void 0){Je.init();let e=window.showLoading;window.showLoading=(t=`Cargando...`)=>{document.querySelector(`.class-timeline`)&&t.includes(`clases`)?Je.showFor(`schedule`):typeof Swal<`u`&&e(t)};let t=window.hideLoading;window.hideLoading=()=>{Je.hideAll(),t()}}}),`serviceWorker`in navigator&&(window.location.hostname===`localhost`?(console.log(`Service Worker no registrado en localhost para evitar problemas de caché.`),navigator.serviceWorker.getRegistrations().then(function(e){for(let t of e)t.unregister()})):window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/app/sw.js`,{scope:`/app/`}).then(e=>{console.log(`Service Worker Registrado: `,e),e.active?typeof initMessaging==`function`&&initMessaging():e.addEventListener(`updatefound`,()=>{let t=e.installing;t.addEventListener(`statechange`,()=>{t.state===`activated`&&typeof initMessaging==`function`&&initMessaging()})})}).catch(e=>console.error(`Error de Service Worker: `,e))}));